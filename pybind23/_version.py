...
input_file = DIR.parent / "include/pybind23/detail/common.h"
regex = re.compile(
    r"""
\#define \s+ PYBIND23_VERSION_MAJOR \s+ (?P<major>\d+) .*?
\#define \s+ PYBIND23_VERSION_MINOR \s+ (?P<minor>\d+) .*?
\#define \s+ PYBIND23_VERSION_PATCH \s+ (?P<patch>\S+)
""",
    re.MULTILINE | re.DOTALL | re.VERBOSE,
)

match = regex.search(input_file.read_text(encoding="utf-8"))
assert match, "Unable to find version in pybind23/detail/common.h"
