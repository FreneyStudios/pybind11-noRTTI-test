/*
    tests/pybind23_cross_module_tests.cpp -- contains tests that require multiple modules
...
PYBIND23_MODULE(pybind23_cross_module_tests, m, py::mod_gil_not_used()) {
    m.doc() = "pybind23 cross-module test module";

    // test_local_bindings.py tests:
    //
    // Definitions here are tested by importing both this module and the
    // relevant pybind23_tests submodule from a test_whatever.py
    ...
    m.def("throw_pybind_value_error", []() { throw py::value_error("pybind23 value error"); });
    m.def("throw_pybind_type_error", []() { throw py::type_error("pybind23 type error"); });
    ...
    // assuming pybind23_tests is already loaded):
