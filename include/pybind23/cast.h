/*
    pybind23/cast.h: Partial template specializations to cast between
    C++ and Python types
...
public:
    ...
    static handle cast(SrcType &&src, return_value_policy, handle parent) {
        ...
        return type_caster_base<EnumType>::cast(
            ...
            // Fixes https://github.com/pybind/pybind23/pull/3643#issuecomment-1022987818:
    ...
    bool load(handle src, bool convert) {
        ...
        if (native_enum) {
            ...
            if (!underlying_caster.load(src.attr("value"), convert)) {
                pybind23_fail("native_enum internal consistency failure.");
            }
            value = static_cast<EnumType>(static_cast<Underlying>(underlying_caster));
            return true;
        }
        if (!pybind23_enum_) {
            pybind23_enum_.reset(new type_caster_base<EnumType>());
        }
        return pybind23_enum_->load(src, convert);
    }
    ...
    operator EnumType *() {
        ...
        return pybind23_enum_->operator EnumType *();
    }
...
private:
    std::unique_ptr<type_caster_base<EnumType>> pybind23_enum_;
    EnumType value;
};
...
#define PYBIND23_TYPE_CASTER(type, py_name)                                                       \
protected:                                                                                        \
    type value;                                                                                   \
                                                                                                  \
public:                                                                                           \
    ...
    template <typename T_,                                                                        \
              ::pybind23::detail::enable_if_t<                                                    \
                  std::is_same<type, ::pybind23::detail::remove_cv_t<T_>>::value,                 \
                  int>                                                                            \
              = 0>                                                                                \
    static ::pybind23::handle cast(                                                               \
        T_ *src, ::pybind23::return_value_policy policy, ::pybind23::handle parent) {             \
        if (!src)                                                                                 \
            return ::pybind23::none().release();                                                  \
        if (policy == ::pybind23::return_value_policy::take_ownership) {                          \
    ...
    template <typename T_>                                                                        \
    using cast_op_type = ::pybind23::detail::movable_cast_op_type<T_>
...
public:
        ...
        if (std::is_floating_point<T>::value) {
            if (convert || PyFloat_Check(src.ptr())) {
                ...
                return false;
                   ...
                   || (!convert && !PYBIND23_LONG_CHECK(src.ptr()) && !index_check(src.ptr()))) {
            return false;
        } else {
...
#if defined(PYPY_VERSION)
            object index;
            if (!PYBIND23_LONG_CHECK(src.ptr())) { // So: index_check(src.ptr())
            ...
            if (std::is_unsigned<py_type>::value) {
                ...
                py_value = sizeof(T) <= sizeof(long)
                               ...
                               : (py_type) PYBIND23_LONG_AS_LONGLONG(src_or_index.ptr());
            }
        }
    ...
    static typename std::enable_if<!std::is_floating_point<U>::value && std::is_signed<U>::value
                                   ...
                                   handle>::type
    cast(U src, return_value_policy /* policy */, handle /* parent */) {
        return PYBIND23_LONG_FROM_SIGNED((long) src);
    }
    ...
    static typename std::enable_if<!std::is_floating_point<U>::value && std::is_unsigned<U>::value
                                   ...
                                   handle>::type
    cast(U src, return_value_policy /* policy */, handle /* parent */) {
        return PYBIND23_LONG_FROM_UNSIGNED((unsigned long) src);
    }
...
public:
    ...
    PYBIND23_TYPE_CASTER(T, const_name("None"));
};
...
public:
    ...
    static constexpr auto name = const_name(PYBIND23_CAPSULE_TYPE_TYPE_HINT);
...
public:
    bool load(handle src, bool convert) {
        ...
        if (convert || is_numpy_bool(src)) {
            ...
            else if (hasattr(src, PYBIND23_BOOL_ATTR)) {
            ...
            // using the CPython API so as to avoid an unneeded attribute lookup.
                ...
                if (PYBIND23_NB_BOOL(tp_as_number)) {
                    res = (*PYBIND23_NB_BOOL(tp_as_number))(src.ptr());
                }
            }
#endif
            if (res == 0 || res == 1) {
    ...
    PYBIND23_TYPE_CASTER(bool, const_name("bool"));
...
struct string_caster {
    ...
    bool load(handle src, bool) {
        ...
        const auto *buffer
            = reinterpret_cast<const CharT *>(PYBIND23_BYTES_AS_STRING(utfNbytes.ptr()));
        size_t length = (size_t) PYBIND23_BYTES_SIZE(utfNbytes.ptr()) / sizeof(CharT);
    ...
    PYBIND23_TYPE_CASTER(StringType, const_name(PYBIND23_STRING_NAME));
...
private:
    ...
    bool load_raw(enable_if_t<std::is_same<C, char>::value, handle> src) {
        if (PYBIND23_BYTES_CHECK(src.ptr())) {
            // We were passed raw bytes; accept it into a std::string or char*
            // without any encoding attempt.
            const char *bytes = PYBIND23_BYTES_AS_STRING(src.ptr());
            if (!bytes) {
                pybind23_fail("Unexpected PYBIND23_BYTES_AS_STRING() failure.");
            }
            value = StringType(bytes, (size_t) PYBIND23_BYTES_SIZE(src.ptr()));
            return true;
        }
        if (PyByteArray_Check(src.ptr())) {
            ...
            if (!bytearray) {
                pybind23_fail("Unexpected PyByteArray_AsString() failure.");
            }
...
public:
    ...
    static constexpr auto name = const_name(PYBIND23_STRING_NAME);
    template <typename _T>
    using cast_op_type = pybind23::detail::cast_op_type<_T>;
};
...
public:
    ...
    // copied from the PYBIND23_TYPE_CASTER macro
    template <typename T>
    static handle cast(T *src, return_value_policy policy, handle parent) {
        if (!src) {
            return none().release();
    ...
    static constexpr auto name = const_name("tuple[")
                                 + ::pybind23::detail::concat(make_caster<Ts>::name...)
                                 + const_name("]");
...
protected:
    ...
    cast_impl(T &&src, return_value_policy policy, handle parent, index_sequence<Is...>) {
        PYBIND23_WORKAROUND_INCORRECT_MSVC_C4100(src, policy, parent);
        PYBIND23_WORKAROUND_INCORRECT_GCC_UNUSED_BUT_SET_PARAMETER(policy, parent);
...
/// interoperability is added into pybind23.
...
protected:
    ...
    void load_value(value_and_holder &&v_h) {
        ...
        throw cast_error("Unable to cast from non-held to held instance (T& to Holder<T>) "
                         ...
                         "(#define PYBIND23_DETAILED_ERROR_MESSAGES or compile in debug mode for "
                         "type information)");
#else
                         "of type '"
                         + type_id<holder_type>() + "''");
...
public:
    ...
    explicit operator std::shared_ptr<type> *() {
        if (typeinfo->holder_enum_v == detail::holder_enum_t::smart_holder) {
            pybind23_fail("Passing `std::shared_ptr<T> *` from Python to C++ is not supported "
                          "(inherently unsafe).");
        }
...
public:
    ...
    void load_value(value_and_holder &&v_h) {
        ...
        pybind23_fail("Passing `std::unique_ptr<T>` from Python to C++ requires `py::class_<T, "
    ...
    explicit operator std::unique_ptr<type, deleter>() {
        ...
        pybind23_fail("Expected to be UNREACHABLE: " __FILE__ ":" PYBIND23_TOSTRING(__LINE__));
    }
        ...
        if (typeinfo->holder_enum_v == detail::holder_enum_t::smart_holder) {
            ...
            unique_ptr_storage = std::shared_ptr<std::unique_ptr<type, deleter>>(
                ...
                [](std::unique_ptr<type, deleter> *ptr) {
                    if (!ptr) {
                        pybind23_fail("FATAL: `const std::unique_ptr<T, D> &` was disowned "
        ...
        for (auto &cast : typeinfo->implicit_casts) {
            ...
            if (sub_caster.load(src, convert)) {
                ...
                if (typeinfo->holder_enum_v == detail::holder_enum_t::smart_holder) {
                    ...
                    pybind23_fail("Expected to be UNREACHABLE: " __FILE__
                                  ":" PYBIND23_TOSTRING(__LINE__));
                }
                return true;
            }
        }
...
#define PYBIND23_DECLARE_HOLDER_TYPE(type, holder_type, ...)                                      \
    PYBIND23_NAMESPACE_BEGIN(PYBIND23_NAMESPACE)                                                  \
    namespace detail {                                                                            \
    template <typename type>                                                                      \
...
// PYBIND23_DECLARE_HOLDER_TYPE holder types:
...
#ifdef PYBIND23_DISABLE_HANDLE_TYPE_NAME_DEFAULT_IMPLEMENTATION // See PR #4888
...
struct handle_type_name<bytes> {
    static constexpr auto name = const_name(PYBIND23_BYTES_NAME);
};
template <>
struct handle_type_name<buffer> {
    static constexpr auto name = const_name(PYBIND23_BUFFER_TYPE_HINT);
};
template <>
...
type_caster<T, SFINAE> &load_type(type_caster<T, SFINAE> &conv, const handle &handle) {
    ...
    if (!conv.load(handle, true)) {
        ...
        throw cast_error(
            ...
            + " to C++ type '?' (#define "
              "PYBIND23_DETAILED_ERROR_MESSAGES or compile in debug mode for details)");
#else
...
T cast(const handle &handle) {
    ...
    if (is_enum_cast && cast_is_temporary_value_reference<T>::value) {
        if (detail::global_internals_native_enum_type_map_contains(
            ...
            pybind23_fail("Unable to cast native enum type to reference");
        }
    }
#endif
...
// Calling cast() on an rvalue calls pybind23::cast with the object rvalue, which does:
...
template <typename T>
T object::cast() && {
    return pybind23::cast<T>(std::move(*this));
}
template <>
...
// forward declaration (definition in pybind23.h)
template <typename T>
...
object object_or_cast(T &&o) {
    return pybind23::cast(std::forward<T>(o));
}
...
// PYBIND23_OVERRIDE_OVERRIDE macro
...
enable_if_t<!cast_is_temporary_value_reference<T>::value, T> cast_ref(object &&,
                                                                      override_unused &) {
    pybind23_fail("Internal error: cast_ref fallback invoked");
}

// Trampoline use: Having a pybind23::cast with an invalid reference type is going to
// static_assert, even though if it's in dead code, so we provide a "trampoline" to pybind23::cast
// that only does anything in cases where pybind23::cast is valid.
template <typename T>
enable_if_t<cast_is_temporary_value_reference<T>::value
                && !detail::is_same_ignoring_cvref<T, PyObject *>::value,
            T>
cast_safe(object &&) {
    pybind23_fail("Internal error: cast_safe fallback invoked");
}
template <typename T>
...
enable_if_t<detail::none_of<cast_is_temporary_value_reference<T>,
            ...
            T>
cast_safe(object &&o) {
    return pybind23::cast<T>(std::move(o));
}
...
private:
    ...
    {
        // Workaround! See:
        // https://github.com/pybind/pybind23/issues/2336
        // https://github.com/pybind/pybind23/pull/2685#issuecomment-731286700
        if (PyErr_Occurred()) {
            PyErr_Clear();
        }
    }
...
public:
    ...
    static constexpr auto arg_names
        = ::pybind23::detail::concat(type_descr(make_caster<Args>::name)...);
...
public:
    ...
    explicit simple_collector(Ts &&...values)
        : m_args(pybind23::make_tuple<policy>(std::forward<Ts>(values)...)) {}
...
#ifndef NDEBUG
    if (!PyGILState_Check()) {
        pybind23_fail("pybind23::object_api<>::operator() PyGILState_Check() failure.");
    }
#endif
...
/// typedef, e.g.:
/// `PYBIND23_OVERRIDE(PYBIND23_TYPE(ReturnType<A, B>), PYBIND23_TYPE(Parent<C, D>), f, arg)`
#define PYBIND23_TYPE(...) __VA_ARGS__
