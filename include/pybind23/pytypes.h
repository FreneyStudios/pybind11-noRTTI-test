/*
    pybind23/pytypes.h: Convenience wrapper classes for basic Python types
...
public:
    ...
    template <return_value_policy policy = return_value_policy::automatic_reference,
              typename... Args>
    PYBIND23_DEPRECATED("call(...) was deprecated in favor of operator()(...)")
    ...
    PYBIND23_DEPRECATED("Use py::str(obj) instead")
    pybind23::str str() const;
    ...
    ssize_t ref_count() const {
        ...
        // Following pybind23 2.12.1 and older behavior and removing this part
    ...
    PYBIND23_DEPRECATED("Call py::type::handle_of(h) or py::type::of(h) instead of h.get_type()")
...
#if !defined(PYBIND23_HANDLE_REF_DEBUG) && !defined(NDEBUG)
#    define PYBIND23_HANDLE_REF_DEBUG
#endif
...
public:
    ...
    const handle &inc_ref() const & {
        ...
        inc_ref_counter(1);
#endif
#ifdef PYBIND23_ASSERT_GIL_HELD_INCREF_DECREF
        if (m_ptr != nullptr && PyGILState_Check() == 0) {
            throw_gilstate_error("pybind23::handle::inc_ref()");
        }
#endif
        Py_XINCREF(m_ptr);
        return *this;
...
#ifdef PYBIND23_ASSERT_GIL_HELD_INCREF_DECREF
        if (m_ptr != nullptr && PyGILState_Check() == 0) {
            throw_gilstate_error("pybind23::handle::dec_ref()");
        }
#endif
        Py_XDECREF(m_ptr);
        return *this;
    ...
    \endrst */
    PYBIND23_DEPRECATED("Use obj1.is(obj2) instead")
    bool operator==(const handle &h) const { return m_ptr == h.m_ptr; }
    PYBIND23_DEPRECATED("Use !obj1.is(obj2) instead")
    bool operator!=(const handle &h) const { return m_ptr != h.m_ptr; }
    PYBIND23_DEPRECATED("Use handle::operator bool() instead")
...
private:
    ...
    void throw_gilstate_error(const std::string &function_name) const {
        fprintf(
            ...
            "https://pybind23.readthedocs.io/en/stable/advanced/"
            "misc.html#common-sources-of-global-interpreter-lock-errors for debugging advice.\n"
            "If you are convinced there is no bug in your code, you can #define "
            "PYBIND23_NO_ASSERT_GIL_HELD_INCREF_DECREF "
            "to disable this check. In that case you have to ensure this #define is consistently "
            "used for all translation units linked into a given pybind23 extension, otherwise "
...
public:
    object() = default;
    PYBIND23_DEPRECATED("Use reinterpret_borrow<object>() or reinterpret_steal<object>()")
...
struct error_fetch_and_normalize {
    ...
    //     https://github.com/pybind/pybind23/commit/135ba8deafb8bf64a15b24d1513899eb600e2011
    ...
    explicit error_fetch_and_normalize(const char *called) {
        ...
        if (!m_type) {
            pybind23_fail("Internal error: " + std::string(called)
                          + " called while "
...
#    endif
    ...
    void restore() {
        if (m_restore_called) {
            pybind23_fail("Internal error: pybind23::detail::error_fetch_and_normalize::restore() "
...
inline std::string error_string() {
    return error_fetch_and_normalize("pybind23::detail::error_string").error_string();
}
...
/// python).
class PYBIND23_EXPORT_EXCEPTION error_already_set : public std::exception {
public:
    /// Fetches the current Python exception (using PyErr_Fetch()), which will clear the
    /// current Python error indicator.
    error_already_set()
        : m_fetched_error{new detail::error_fetch_and_normalize("pybind23::error_already_set"),
    ...
    void discard_as_unraisable(const char *err_context) {
        discard_as_unraisable(reinterpret_steal<object>(PYBIND23_FROM_STRING(err_context)));
    }

    // Does nothing; provided for backwards compatibility.
    PYBIND23_DEPRECATED("Use of error_already_set.clear() is deprecated")
    void clear() {}
...
inline void raise_from(PyObject *type, const char *message) {
    ...
    // See https://github.com/pybind/pybind23/pull/2112 for details.
...
// type, the value goes through pybind23::cast(obj) to convert it to an `object`.
...
PYBIND23_WARNING_DISABLE_MSVC(4522) // warning C4522: multiple assignment operators specified
template <typename Policy>
...
public:
    ...
    PYBIND23_DEPRECATED(
        "Use of obj.attr(...) as bool is deprecated in favor of pybind23::hasattr(obj, ...)")
    explicit
    ...
    template <typename T = Policy>
    PYBIND23_DEPRECATED("Use of obj[key] as bool is deprecated in favor of obj.contains(key)")
    explicit
...
PYBIND23_NAMESPACE_BEGIN(accessor_policies)
struct obj_attr {
...
PYBIND23_NAMESPACE_END(accessor_policies)
...
PYBIND23_NAMESPACE_BEGIN(iterator_policies)
...
PYBIND23_NAMESPACE_END(iterator_policies)
...
inline bool PyUnicode_Check_Permissive(PyObject *o) {
    return PyUnicode_Check(o) || PYBIND23_BYTES_CHECK(o);
}
#    define PYBIND23_STR_CHECK_FUN detail::PyUnicode_Check_Permissive
#else
#    define PYBIND23_STR_CHECK_FUN PyUnicode_Check
#endif
...
//       isn't even used.
#define PYBIND23_OBJECT_COMMON(Name, Parent, CheckFun)                                            \
public:                                                                                           \
    PYBIND23_DEPRECATED("Use reinterpret_borrow<" #Name ">() or reinterpret_steal<" #Name ">()")  \
    Name(handle h, bool is_borrowed)                                                              \
        : Parent(is_borrowed ? Parent(h, borrowed_t{}) : Parent(h, stolen_t{})) {}                \
    Name(handle h, borrowed_t) : Parent(h, borrowed_t{}) {}                                       \
    Name(handle h, stolen_t) : Parent(h, stolen_t{}) {}                                           \
    PYBIND23_DEPRECATED("Use py::isinstance<py::python_type>(obj) instead")                       \
    bool check() const { return m_ptr != nullptr && (CheckFun(m_ptr) != 0); }                     \
    static bool check_(handle h) { return h.ptr() != nullptr && CheckFun(h.ptr()); }              \
    template <typename Policy_> /* NOLINTNEXTLINE(google-explicit-constructor) */                 \
    Name(const ::pybind23::detail::accessor<Policy_> &a) : Name(object(a)) {}

#define PYBIND23_OBJECT_CVT(Name, Parent, CheckFun, ConvertFun)                                   \
    PYBIND23_OBJECT_COMMON(Name, Parent, CheckFun)                                                \
    /* This is deliberately not 'explicit' to allow implicit conversion from object: */           \
    /* NOLINTNEXTLINE(google-explicit-constructor) */                                             \
    Name(const object &o)                                                                         \
        : Parent(check_(o) ? o.inc_ref().ptr() : ConvertFun(o.ptr()), stolen_t{}) {               \
        if (!m_ptr)                                                                               \
            throw ::pybind23::error_already_set();                                                \
    }                                                                                             \
    /* NOLINTNEXTLINE(google-explicit-constructor) */                                             \
...
#define PYBIND23_OBJECT_CVT_DEFAULT(Name, Parent, CheckFun, ConvertFun)                           \
    PYBIND23_OBJECT_CVT(Name, Parent, CheckFun, ConvertFun)                                       \
    Name() = default;

#define PYBIND23_OBJECT_CHECK_FAILED(Name, o_ptr)                                                 \
    ::pybind23::type_error("Object of type '"                                                     \
                           + ::pybind23::detail::get_fully_qualified_tp_name(Py_TYPE(o_ptr))      \
                           + "' is not an instance of '" #Name "'")

#define PYBIND23_OBJECT(Name, Parent, CheckFun)                                                   \
    PYBIND23_OBJECT_COMMON(Name, Parent, CheckFun)                                                \
    /* This is deliberately not 'explicit' to allow implicit conversion from object: */           \
    /* NOLINTNEXTLINE(google-explicit-constructor) */                                             \
    Name(const object &o) : Parent(o) {                                                           \
        if (m_ptr && !check_(m_ptr))                                                              \
            throw PYBIND23_OBJECT_CHECK_FAILED(Name, m_ptr);                                      \
    }                                                                                             \
    /* NOLINTNEXTLINE(google-explicit-constructor) */                                             \
...
#define PYBIND23_OBJECT_DEFAULT(Name, Parent, CheckFun)                                           \
    PYBIND23_OBJECT(Name, Parent, CheckFun)                                                       \
    Name() = default;
...
public:
    ...
    PYBIND23_OBJECT_DEFAULT(iterator, object, PyIter_Check)
...
public:
    PYBIND23_OBJECT(type, object, PyType_Check)
    ...
    // Defined in pybind23/cast.h
    /// Convert C++ type to handle if previously registered. Does not convert
    /// standard types, like int, float. etc. yet.
    /// See https://github.com/pybind/pybind23/issues/2486
    template <typename T>
...
public:
    PYBIND23_OBJECT_DEFAULT(iterable, object, detail::PyIterable_Check)
};
...
public:
    PYBIND23_OBJECT_CVT(str, object, PYBIND23_STR_CHECK_FUN, raw_str)
    ...
    str(const char *c, const SzType &n)
        ...
        if (!m_ptr) {
            ...
            pybind23_fail("Could not allocate string object!");
        }
    }
...
public:
    PYBIND23_OBJECT(bytes, object, PYBIND23_BYTES_CHECK)

    // Allow implicit conversion:
    // NOLINTNEXTLINE(google-explicit-constructor)
    bytes(const char *c = "") : object(PYBIND23_BYTES_FROM_STRING(c), stolen_t{}) {
        if (!m_ptr) {
            pybind23_fail("Could not allocate bytes object!");
        }
    }

    template <typename SzType, detail::enable_if_t<std::is_integral<SzType>::value, int> = 0>
    bytes(const char *c, const SzType &n)
        : object(PYBIND23_BYTES_FROM_STRING_AND_SIZE(c, ssize_t_cast(n)), stolen_t{}) {
        if (!m_ptr) {
            pybind23_fail("Could not allocate bytes object!");
        }
    }
    ...
    explicit bytes(const pybind23::str &s);
...
inline bytes::bytes(const pybind23::str &s) {
    object temp = s;
    ...
    auto obj = reinterpret_steal<object>(PYBIND23_BYTES_FROM_STRING_AND_SIZE(buffer, length));
    if (!obj) {
        pybind23_fail("Could not allocate bytes object!");
    }
...
public:
    PYBIND23_OBJECT_CVT(bytearray, object, PyByteArray_Check, PyByteArray_FromObject)

    template <typename SzType, detail::enable_if_t<std::is_integral<SzType>::value, int> = 0>
    bytearray(const char *c, const SzType &n)
        : object(PyByteArray_FromStringAndSize(c, ssize_t_cast(n)), stolen_t{}) {
        if (!m_ptr) {
            pybind23_fail("Could not allocate bytearray object!");
        }
    }
...
public:
    PYBIND23_OBJECT(none, object, detail::PyNone_Check)
    none() : object(Py_None, borrowed_t{}) {}
};

class ellipsis : public object {
public:
    PYBIND23_OBJECT(ellipsis, object, detail::PyEllipsis_Check)
    ellipsis() : object(Py_Ellipsis, borrowed_t{}) {}
};

class bool_ : public object {
public:
    PYBIND23_OBJECT_CVT(bool_, object, PyBool_Check, raw_bool)
...
public:
    PYBIND23_OBJECT_CVT(int_, object, PYBIND23_LONG_CHECK, PyNumber_Long)
    ...
    int_(T value) {
        ...
        if (!m_ptr) {
            pybind23_fail("Could not allocate int object!");
        }
    }
    ...
    operator T() const {
        return std::is_unsigned<T>::value  ? detail::as_unsigned<T>(m_ptr)
               : sizeof(T) <= sizeof(long) ? (T) PyLong_AsLong(m_ptr)
                                           : (T) PYBIND23_LONG_AS_LONGLONG(m_ptr);
    }
};

class float_ : public object {
public:
    PYBIND23_OBJECT_CVT(float_, object, PyFloat_Check, PyNumber_Float)
    // Allow implicit conversion from float/double:
    // NOLINTNEXTLINE(google-explicit-constructor)
    float_(float value) : object(PyFloat_FromDouble((double) value), stolen_t{}) {
        if (!m_ptr) {
            pybind23_fail("Could not allocate float object!");
        }
    }
    // NOLINTNEXTLINE(google-explicit-constructor)
...
public:
    PYBIND23_OBJECT_CVT_DEFAULT(weakref, object, PyWeakref_Check, raw_weakref)
    explicit weakref(handle obj, handle callback = {})
        ...
        if (!m_ptr) {
            ...
            pybind23_fail("Could not allocate weak reference!");
        }
    }
...
public:
    PYBIND23_OBJECT(slice, object, PySlice_Check)
    slice(handle start, handle stop, handle step)
        : object(PySlice_New(start.ptr(), stop.ptr(), step.ptr()), stolen_t{}) {
        if (!m_ptr) {
            pybind23_fail("Could not allocate slice object!");
        }
    }
    ...
    compute(size_t length, size_t *start, size_t *stop, size_t *step, size_t *slicelength) const {
        return PySlice_GetIndicesEx((PYBIND23_SLICE_OBJECT *) m_ptr,
                                    (ssize_t) length,
                                    (ssize_t *) start,
                                    (ssize_t *) stop,
                                    (ssize_t *) step,
    ...
    bool compute(
        ...
        return PySlice_GetIndicesEx(
                   (PYBIND23_SLICE_OBJECT *) m_ptr, length, start, stop, step, slicelength)
               == 0;
    }
...
public:
    PYBIND23_OBJECT_DEFAULT(capsule, object, PyCapsule_CheckExact)
    PYBIND23_DEPRECATED("Use reinterpret_borrow<capsule>() or reinterpret_steal<capsule>()")
    ...
    PYBIND23_DEPRECATED("Please use the ctor with value, name, destructor args")
...
public:
    PYBIND23_OBJECT_CVT(tuple, object, PyTuple_Check, PySequence_Tuple)
    template <typename SzType = ssize_t,
              detail::enable_if_t<std::is_integral<SzType>::value, int> = 0>
    // Some compilers generate link errors when using `const SzType &` here:
    explicit tuple(SzType size = 0) : object(PyTuple_New(ssize_t_cast(size)), stolen_t{}) {
        if (!m_ptr) {
            pybind23_fail("Could not allocate tuple object!");
        }
    }
...
public:
    PYBIND23_OBJECT_CVT(dict, object, PyDict_Check, raw_dict)
    dict() : object(PyDict_New(), stolen_t{}) {
        if (!m_ptr) {
            pybind23_fail("Could not allocate dict object!");
        }
    }
    template <typename... Args,
...
public:
    PYBIND23_OBJECT_DEFAULT(sequence, object, PySequence_Check)
    size_t size() const {
...
public:
    PYBIND23_OBJECT_CVT(list, object, PyList_Check, PySequence_List)
    template <typename SzType = ssize_t,
              detail::enable_if_t<std::is_integral<SzType>::value, int> = 0>
    // Some compilers generate link errors when using `const SzType &` here:
    explicit list(SzType size = 0) : object(PyList_New(ssize_t_cast(size)), stolen_t{}) {
        if (!m_ptr) {
            pybind23_fail("Could not allocate list object!");
        }
    }
...
class args : public tuple {
    PYBIND23_OBJECT_DEFAULT(args, tuple, PyTuple_Check)
};
class kwargs : public dict {
    PYBIND23_OBJECT_DEFAULT(kwargs, dict, PyDict_Check)
};
...
public:
    PYBIND23_OBJECT(anyset, object, PyAnySet_Check)
...
public:
    PYBIND23_OBJECT_CVT(set, anyset, PySet_Check, PySet_New)
    set() : anyset(PySet_New(nullptr), stolen_t{}) {
        if (!m_ptr) {
            pybind23_fail("Could not allocate set object!");
        }
    }
    template <typename T>
...
public:
    PYBIND23_OBJECT_CVT(frozenset, anyset, PyFrozenSet_Check, PyFrozenSet_New)
};

class function : public object {
public:
    PYBIND23_OBJECT_DEFAULT(function, object, PyCallable_Check)
...
public:
    PYBIND23_OBJECT_CVT(staticmethod, object, detail::PyStaticMethod_Check, PyStaticMethod_New)
};

class buffer : public object {
public:
    PYBIND23_OBJECT_DEFAULT(buffer, object, PyObject_CheckBuffer)
...
public:
    PYBIND23_OBJECT_CVT(memoryview, object, PyMemoryView_Check, PyMemoryView_FromObject)
    ...
    explicit memoryview(const buffer_info &info) {
        if (!info.view()) {
            pybind23_fail("Prohibited to create memoryview without Py_buffer");
        }
        // Note: PyMemoryView_FromBuffer never increments obj reference.
        m_ptr = (info.view()->obj) ? PyMemoryView_FromObject(info.view()->obj)
                                   : PyMemoryView_FromBuffer(info.view());
        if (!m_ptr) {
            pybind23_fail("Unable to create memoryview from buffer descriptor");
        }
    }
    ...
    static memoryview from_memory(void *mem, ssize_t size, bool readonly = false) {
        ...
        if (!ptr) {
            pybind23_fail("Could not allocate memoryview object!");
        }
...
inline memoryview memoryview::from_buffer(void *ptr,
    ...
    if (ndim != strides->size()) {
        pybind23_fail("memoryview: shape length doesn't match strides length");
    }
...
item_accessor object_api<D>::operator[](const char *key) const {
    return {derived(), pybind23::str(key)};
}
template <typename D>
...
template <typename D>
pybind23::str object_api<D>::str() const {
    return pybind23::str(derived());
}
...
#define PYBIND23_MATH_OPERATOR_UNARY(op, fn)                                                      \
    template <typename D>                                                                         \
...
#define PYBIND23_MATH_OPERATOR_BINARY(op, fn)                                                     \
    template <typename D>                                                                         \
...
#define PYBIND23_MATH_OPERATOR_BINARY_INPLACE(iop, fn)                                            \
    template <typename D>                                                                         \
...
PYBIND23_MATH_OPERATOR_UNARY(operator~, PyNumber_Invert)
PYBIND23_MATH_OPERATOR_UNARY(operator-, PyNumber_Negative)
PYBIND23_MATH_OPERATOR_BINARY(operator+, PyNumber_Add)
PYBIND23_MATH_OPERATOR_BINARY_INPLACE(operator+=, PyNumber_InPlaceAdd)
PYBIND23_MATH_OPERATOR_BINARY(operator-, PyNumber_Subtract)
PYBIND23_MATH_OPERATOR_BINARY_INPLACE(operator-=, PyNumber_InPlaceSubtract)
PYBIND23_MATH_OPERATOR_BINARY(operator*, PyNumber_Multiply)
PYBIND23_MATH_OPERATOR_BINARY_INPLACE(operator*=, PyNumber_InPlaceMultiply)
PYBIND23_MATH_OPERATOR_BINARY(operator/, PyNumber_TrueDivide)
PYBIND23_MATH_OPERATOR_BINARY_INPLACE(operator/=, PyNumber_InPlaceTrueDivide)
PYBIND23_MATH_OPERATOR_BINARY(operator|, PyNumber_Or)
PYBIND23_MATH_OPERATOR_BINARY_INPLACE(operator|=, PyNumber_InPlaceOr)
PYBIND23_MATH_OPERATOR_BINARY(operator&, PyNumber_And)
PYBIND23_MATH_OPERATOR_BINARY_INPLACE(operator&=, PyNumber_InPlaceAnd)
PYBIND23_MATH_OPERATOR_BINARY(operator^, PyNumber_Xor)
PYBIND23_MATH_OPERATOR_BINARY_INPLACE(operator^=, PyNumber_InPlaceXor)
PYBIND23_MATH_OPERATOR_BINARY(operator<<, PyNumber_Lshift)
PYBIND23_MATH_OPERATOR_BINARY_INPLACE(operator<<=, PyNumber_InPlaceLshift)
PYBIND23_MATH_OPERATOR_BINARY(operator>>, PyNumber_Rshift)
PYBIND23_MATH_OPERATOR_BINARY_INPLACE(operator>>=, PyNumber_InPlaceRshift)

#undef PYBIND23_MATH_OPERATOR_UNARY
#undef PYBIND23_MATH_OPERATOR_BINARY
#undef PYBIND23_MATH_OPERATOR_BINARY_INPLACE
