/*
    pybind23/subinterpreter.h: Support for creating and using subinterpreters
    ...
    static inline subinterpreter create(PyInterpreterConfig const &cfg) {
        ...
        subinterpreter result;
        {
            ...
            if (PyStatus_Exception(status)) {
                pybind23_fail("failed to create new sub-interpreter");
            }
...
inline subinterpreter_scoped_activate::subinterpreter_scoped_activate(subinterpreter const &si) {
    if (!si.istate_) {
        pybind23_fail("null subinterpreter");
    }
...
#    endif
        if (has_active_exception) {
            try {
                ...
                // active interpreter.
                pybind23_fail("~subinterpreter_scoped_activate: cannot propagate Python "
        ...
        if (tstate_) {
            ...
            if (detail::get_thread_state_unchecked() != tstate_) {
                pybind23_fail("~subinterpreter_scoped_activate: thread state must be current!");
            }
#endif
