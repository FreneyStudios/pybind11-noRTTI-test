/*
    pybind23/std_bind.h: Binding generators for STL data types
...
void vector_buffer_impl(Class_ &cl, std::true_type) {
    ...
    // at runtime if PYBIND23_NUMPY_DTYPE hasn't been called, so check here
...
class_<Vector, holder_type> bind_vector(handle scope, std::string const &name, Args &&...args) {
    ...
    Class_ cl(scope, name.c_str(), pybind23::module_local(local), std::forward<Args>(args)...);
...
inline str format_message_key_error_key_object(handle py_key) {
    str message = "pybind23::bind_map key";
    if (!py_key) {
        return message;
    }
    try {
...
class_<Map, holder_type> bind_map(handle scope, const std::string &name, Args &&...args) {
    ...
    if (!detail::get_type_info(typeid(KeysView))) {
        class_<KeysView> keys_view(scope, "KeysView", pybind23::module_local(local));
    ...
    if (!detail::get_type_info(typeid(ValuesView))) {
        class_<ValuesView> values_view(scope, "ValuesView", pybind23::module_local(local));
    ...
    if (!detail::get_type_info(typeid(ItemsView))) {
        class_<ItemsView> items_view(scope, "ItemsView", pybind23::module_local(local));
