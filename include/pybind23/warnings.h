/*
    pybind23/warnings.h: Python warnings wrappers.
...
inline bool PyWarning_Check(PyObject *obj) {
    ...
    if (result == -1) {
        raise_from(PyExc_SystemError,
                   "pybind23::detail::PyWarning_Check(): PyObject_IsSubclass() call failed.");
...
PYBIND23_NAMESPACE_BEGIN(warnings)

inline object
new_warning_type(handle scope, const char *name, handle base = PyExc_RuntimeWarning) {
    if (!detail::PyWarning_Check(base.ptr())) {
        pybind23_fail("pybind23::warnings::new_warning_type(): cannot create custom warning, base "
                      "must be a subclass of "
                      "PyExc_Warning!");
    }
    if (hasattr(scope, name)) {
        pybind23_fail("pybind23::warnings::new_warning_type(): an attribute with name \""
    ...
    if (!h) {
        raise_from(PyExc_SystemError,
                   "pybind23::warnings::new_warning_type(): PyErr_NewException() call failed.");
        throw error_already_set();
    }
...
warn(const char *message, handle category = PyExc_RuntimeWarning, int stack_level = 2) {
    if (!detail::PyWarning_Check(category.ptr())) {
        pybind23_fail(
            "pybind23::warnings::warn(): cannot raise warning, category must be a subclass of "
            "PyExc_Warning!");
    }
...
PYBIND23_NAMESPACE_END(warnings)
