/*
    pybind23/numpy.h: Basic NumPy support, vectorize() wrapper
...
#if defined(PYBIND23_NUMPY_1_ONLY)
#    error "PYBIND23_NUMPY_1_ONLY is no longer supported (see PR #5595)."
#endif
...
static_assert(sizeof(::pybind23::ssize_t) == sizeof(Py_intptr_t), "ssize_t != Py_intptr_t");
...
struct numpy_internals {
    ...
    numpy_type_info *get_type_info(const std::type_info &tinfo, bool throw_if_missing = true) {
        ...
        if (throw_if_missing) {
            pybind23_fail(std::string("NumPy type info missing for ") + tinfo.name());
        }
        return nullptr;
    }
...
PYBIND23_NOINLINE void load_numpy_internals(numpy_internals *&ptr) {
...
PYBIND23_NOINLINE module_ import_numpy_core_submodule(const char *submodule_name) {
...
struct npy_api {
    ...
    static npy_api &get() {
        PYBIND23_CONSTINIT static gil_safe_call_once_and_store<npy_api> storage;
...
#define DECL_NPY_API(Func) api.Func##_ = (decltype(api.Func##_)) api_ptr[API_##Func];
        ...
        if (api.PyArray_RUNTIME_VERSION_ < 0x7) {
            pybind23_fail("pybind23 numpy support requires numpy >= 1.7.0");
        }
...
#define PYBIND23_NUMPY_SCALAR_IMPL(ctype_, typenum_)                                              \
    template <>                                                                                   \
...
// boolean type
PYBIND23_NUMPY_SCALAR_IMPL(bool, NPY_BOOL);

// character types
PYBIND23_NUMPY_SCALAR_IMPL(char, NPY_CHAR);
PYBIND23_NUMPY_SCALAR_IMPL(signed char, NPY_BYTE);
PYBIND23_NUMPY_SCALAR_IMPL(unsigned char, NPY_UBYTE);

// signed integer types
PYBIND23_NUMPY_SCALAR_IMPL(std::int16_t, NPY_INT16);
PYBIND23_NUMPY_SCALAR_IMPL(std::int32_t, NPY_INT32);
PYBIND23_NUMPY_SCALAR_IMPL(std::int64_t, NPY_INT64);

// unsigned integer types
PYBIND23_NUMPY_SCALAR_IMPL(std::uint16_t, NPY_UINT16);
PYBIND23_NUMPY_SCALAR_IMPL(std::uint32_t, NPY_UINT32);
PYBIND23_NUMPY_SCALAR_IMPL(std::uint64_t, NPY_UINT64);

// floating point types
PYBIND23_NUMPY_SCALAR_IMPL(float, NPY_FLOAT);
PYBIND23_NUMPY_SCALAR_IMPL(double, NPY_DOUBLE);
PYBIND23_NUMPY_SCALAR_IMPL(long double, NPY_LONGDOUBLE);

// complex types
PYBIND23_NUMPY_SCALAR_IMPL(std::complex<float>, NPY_CFLOAT);
PYBIND23_NUMPY_SCALAR_IMPL(std::complex<double>, NPY_CDOUBLE);
PYBIND23_NUMPY_SCALAR_IMPL(std::complex<long double>, NPY_CLONGDOUBLE);

#undef PYBIND23_NUMPY_SCALAR_IMPL
...
struct array_info<std::array<T, N>> {
    ...
    static constexpr auto extents = const_name<array_info<T>::is_array>(
        ::pybind23::detail::concat(const_name<N>(), array_info<T>::extents), const_name<N>());
};
...
struct type_caster<numpy_scalar<T>> {
    ...
    PYBIND23_TYPE_CASTER(numpy_scalar<T>, type_info::name);
...
public:
    PYBIND23_OBJECT_DEFAULT(dtype, object, detail::npy_api::get().PyArrayDescr_Check_)

    explicit dtype(const buffer_info &info) {
        dtype descr(_dtype_from_pep3118()(pybind23::str(info.format)));
    ...
    explicit dtype(const pybind23::str &format) : dtype(from_args(format)) {}

    explicit dtype(const std::string &format) : dtype(pybind23::str(format)) {}

    explicit dtype(const char *format) : dtype(pybind23::str(format)) {}
    ...
    dtype(list names, list formats, list offsets, ssize_t itemsize) {
        ...
        args["itemsize"] = pybind23::int_(itemsize);
...
private:
    static object &_dtype_from_pep3118() {
        PYBIND23_CONSTINIT static gil_safe_call_once_and_store<object> storage;
        return storage
    ...
    dtype strip_padding(ssize_t itemsize) {
        ...
        struct field_descr {
            pybind23::str name;
            object format;
            pybind23::int_ offset;
            field_descr(pybind23::str &&name, object &&format, pybind23::int_ &&offset)
        ...
        for (auto field : field_dict.attr("items")()) {
            ...
            auto name = spec[0].cast<pybind23::str>();
            auto spec_fo = spec[1].cast<tuple>();
            auto format = spec_fo[0].cast<dtype>();
            auto offset = spec_fo[1].cast<pybind23::int_>();
...
public:
    PYBIND23_OBJECT_CVT(array, buffer, detail::npy_api::get().PyArray_Check_, raw_array)
    ...
    array(const pybind23::dtype &dt,
        ...
        if (ndim != strides->size()) {
            pybind23_fail("NumPy: shape ndim doesn't match strides ndim");
        }
        auto descr = dt;
    ...
    array(const pybind23::dtype &dt, T count, const void *ptr = nullptr, handle base = handle())
        : array(dt, {{count}}, ptr, base) {}

    template <typename T>
    array(ShapeContainer shape, StridesContainer strides, const T *ptr, handle base = handle())
        : array(pybind23::dtype::of<T>(),
                std::move(shape),
                std::move(strides),
    ...
    explicit array(const buffer_info &info, handle base = handle())
        : array(pybind23::dtype(info), info.shape, info.strides, info.ptr, base) {}

    /// Array descriptor (dtype)
    pybind23::dtype dtype() const {
        return reinterpret_borrow<pybind23::dtype>(detail::array_proxy(m_ptr)->descr);
    }
    ...
    array view(const std::string &dtype) {
        ...
        auto new_view = reinterpret_steal<array>(api.PyArray_View_(
            m_ptr, dtype::from_args(pybind23::str(dtype)).release().ptr(), nullptr));
        if (!new_view) {
            throw error_already_set();
        }
        return new_view;
...
protected:
    ...
    static PyObject *raw_array(PyObject *ptr, int ExtraFlags = 0) {
        if (ptr == nullptr) {
            set_error(PyExc_ValueError, "cannot create a pybind23::array from a nullptr");
            return nullptr;
        }
...
public:
    ...
    PYBIND23_DEPRECATED("Use array_t<T>::ensure() instead")
...
protected:
    ...
    static PyObject *raw_array_t(PyObject *ptr) {
        if (ptr == nullptr) {
            set_error(PyExc_ValueError, "cannot create a pybind23::array_t from a nullptr");
            return nullptr;
        }
...
struct pyobject_caster<array_t<T, ExtraFlags>> {
    ...
    PYBIND23_TYPE_CASTER(type, handle_type_name<type>::name);
};
...
public:
    ...
    static pybind23::dtype dtype() { return pybind23::dtype(/*typenum*/ value); }
};
...
#define PYBIND23_DECL_CHAR_FMT                                                                    \
    static constexpr auto name = const_name("S") + const_name<N>();                               \
    static pybind23::dtype dtype() {                                                              \
        return pybind23::dtype(std::string("S") + std::to_string(N));                             \
    }
template <size_t N>
struct npy_format_descriptor<char[N]> {
    PYBIND23_DECL_CHAR_FMT
};
template <size_t N>
struct npy_format_descriptor<std::array<char, N>> {
    PYBIND23_DECL_CHAR_FMT
};
#undef PYBIND23_DECL_CHAR_FMT
...
public:
    ...
    static pybind23::dtype dtype() {
        list shape;
        array_info<T>::append_extents(shape);
        return pybind23::dtype::from_args(
            pybind23::make_tuple(base_descr::dtype(), std::move(shape)));
    }
};
...
public:
    ...
    static pybind23::dtype dtype() { return base_descr::dtype(); }
};
...
PYBIND23_NOINLINE void register_structured_dtype(any_container<field_descriptor> fields,
    ...
    if (numpy_internals.get_type_info(tinfo, false)) {
        pybind23_fail("NumPy: dtype is already registered");
    }
    ...
    for (auto &field : ordered_fields) {
        if (!field.descr) {
            pybind23_fail(std::string("NumPy: unsupported field dtype: `") + field.name + "` @ "
                          + tinfo.name());
        }
        names.append(pybind23::str(field.name));
        formats.append(field.descr);
        offsets.append(pybind23::int_(field.offset));
    }
    auto *dtype_ptr
        = pybind23::dtype(std::move(names), std::move(formats), std::move(offsets), itemsize)
              .release()
              .ptr();
    ...
    if (!api.PyArray_EquivTypes_(dtype_ptr, arr.dtype().ptr())) {
        pybind23_fail("NumPy: invalid buffer descriptor!");
    }
...
struct npy_format_descriptor {
    ...
    static pybind23::dtype dtype() { return reinterpret_borrow<pybind23::dtype>(dtype_ptr()); }
...
#    define PYBIND23_NUMPY_DTYPE(Type, ...) ((void) 0)
#    define PYBIND23_NUMPY_DTYPE_EX(Type, ...) ((void) 0)
#else

#    define PYBIND23_FIELD_DESCRIPTOR_EX(T, Field, Name)                                          \
        ::pybind23::detail::field_descriptor {                                                    \
            Name, offsetof(T, Field), sizeof(decltype(std::declval<T>().Field)),                  \
                ::pybind23::format_descriptor<decltype(std::declval<T>().Field)>::format(),       \
                ::pybind23::detail::npy_format_descriptor<                                        \
                    decltype(std::declval<T>().Field)>::dtype()                                   \
        }

// Extract name, offset and format descriptor for a struct field
#    define PYBIND23_FIELD_DESCRIPTOR(T, Field) PYBIND23_FIELD_DESCRIPTOR_EX(T, Field, #Field)

// The main idea of this macro is borrowed from https://github.com/swansontec/map-macro
// (C) William Swanson, Paul Fultz
#    define PYBIND23_EVAL0(...) __VA_ARGS__
#    define PYBIND23_EVAL1(...) PYBIND23_EVAL0(PYBIND23_EVAL0(PYBIND23_EVAL0(__VA_ARGS__)))
#    define PYBIND23_EVAL2(...) PYBIND23_EVAL1(PYBIND23_EVAL1(PYBIND23_EVAL1(__VA_ARGS__)))
#    define PYBIND23_EVAL3(...) PYBIND23_EVAL2(PYBIND23_EVAL2(PYBIND23_EVAL2(__VA_ARGS__)))
#    define PYBIND23_EVAL4(...) PYBIND23_EVAL3(PYBIND23_EVAL3(PYBIND23_EVAL3(__VA_ARGS__)))
#    define PYBIND23_EVAL(...) PYBIND23_EVAL4(PYBIND23_EVAL4(PYBIND23_EVAL4(__VA_ARGS__)))
#    define PYBIND23_MAP_END(...)
#    define PYBIND23_MAP_OUT
#    define PYBIND23_MAP_COMMA ,
#    define PYBIND23_MAP_GET_END() 0, PYBIND23_MAP_END
#    define PYBIND23_MAP_NEXT0(test, next, ...) next PYBIND23_MAP_OUT
#    define PYBIND23_MAP_NEXT1(test, next) PYBIND23_MAP_NEXT0(test, next, 0)
#    define PYBIND23_MAP_NEXT(test, next) PYBIND23_MAP_NEXT1(PYBIND23_MAP_GET_END test, next)
#    if defined(_MSC_VER)                                                                         \
        && !defined(__clang__) // MSVC is not as eager to expand macros, hence this workaround
#        define PYBIND23_MAP_LIST_NEXT1(test, next)                                               \
            PYBIND23_EVAL0(PYBIND23_MAP_NEXT0(test, PYBIND23_MAP_COMMA next, 0))
#    else
#        define PYBIND23_MAP_LIST_NEXT1(test, next)                                               \
            PYBIND23_MAP_NEXT0(test, PYBIND23_MAP_COMMA next, 0)
#    endif
#    define PYBIND23_MAP_LIST_NEXT(test, next)                                                    \
        PYBIND23_MAP_LIST_NEXT1(PYBIND23_MAP_GET_END test, next)
#    define PYBIND23_MAP_LIST0(f, t, x, peek, ...)                                                \
        f(t, x) PYBIND23_MAP_LIST_NEXT(peek, PYBIND23_MAP_LIST1)(f, t, peek, __VA_ARGS__)
#    define PYBIND23_MAP_LIST1(f, t, x, peek, ...)                                                \
        f(t, x) PYBIND23_MAP_LIST_NEXT(peek, PYBIND23_MAP_LIST0)(f, t, peek, __VA_ARGS__)
// PYBIND23_MAP_LIST(f, t, a1, a2, ...) expands to f(t, a1), f(t, a2), ...
#    define PYBIND23_MAP_LIST(f, t, ...)                                                          \
        PYBIND23_EVAL(PYBIND23_MAP_LIST1(f, t, __VA_ARGS__, (), 0))

#    define PYBIND23_NUMPY_DTYPE(Type, ...)                                                       \
        ::pybind23::detail::npy_format_descriptor<Type>::register_dtype(                          \
            ::std::vector<::pybind23::detail::field_descriptor>{                                  \
                PYBIND23_MAP_LIST(PYBIND23_FIELD_DESCRIPTOR, Type, __VA_ARGS__)})

#    if defined(_MSC_VER) && !defined(__clang__)
#        define PYBIND23_MAP2_LIST_NEXT1(test, next)                                              \
            PYBIND23_EVAL0(PYBIND23_MAP_NEXT0(test, PYBIND23_MAP_COMMA next, 0))
#    else
#        define PYBIND23_MAP2_LIST_NEXT1(test, next)                                              \
            PYBIND23_MAP_NEXT0(test, PYBIND23_MAP_COMMA next, 0)
#    endif
#    define PYBIND23_MAP2_LIST_NEXT(test, next)                                                   \
        PYBIND23_MAP2_LIST_NEXT1(PYBIND23_MAP_GET_END test, next)
#    define PYBIND23_MAP2_LIST0(f, t, x1, x2, peek, ...)                                          \
        f(t, x1, x2) PYBIND23_MAP2_LIST_NEXT(peek, PYBIND23_MAP2_LIST1)(f, t, peek, __VA_ARGS__)
#    define PYBIND23_MAP2_LIST1(f, t, x1, x2, peek, ...)                                          \
        f(t, x1, x2) PYBIND23_MAP2_LIST_NEXT(peek, PYBIND23_MAP2_LIST0)(f, t, peek, __VA_ARGS__)
// PYBIND23_MAP2_LIST(f, t, a1, a2, ...) expands to f(t, a1, a2), f(t, a3, a4), ...
#    define PYBIND23_MAP2_LIST(f, t, ...)                                                         \
        PYBIND23_EVAL(PYBIND23_MAP2_LIST1(f, t, __VA_ARGS__, (), 0))

#    define PYBIND23_NUMPY_DTYPE_EX(Type, ...)                                                    \
        ::pybind23::detail::npy_format_descriptor<Type>::register_dtype(                          \
            ::std::vector<::pybind23::detail::field_descriptor>{                                  \
                PYBIND23_MAP2_LIST(PYBIND23_FIELD_DESCRIPTOR_EX, Type, __VA_ARGS__)})
    ...
    for (size_t i = 0; i < N; ++i) {
        ...
        for (auto shape_iter = buffers[i].shape.rbegin(); shape_iter != end;
            ...
            if (dim_size_out == 1) {
                ...
                pybind23_fail("pybind23::vectorize: incompatible size/dimension of inputs!");
            }
        }
    }
...
private:
    ...
    static_assert(
        NVectorized >= 1,
        "pybind23::vectorize(...) requires a function with at least one vectorizable argument");
...
private:
    ...
    object run(typename vectorize_arg<Args>::type &...args,
        ...
        if (size == 1 && ndim == 0) {
            PYBIND23_EXPAND_SIDE_EFFECTS(params[VIndex] = buffers[BIndex].ptr);
            return cast(
    ...
    void apply_broadcast(std::array<buffer_info, NVectorized> &buffers,
        ...
        for (size_t i = 0; i < size; ++i, ++input_iter) {
            PYBIND23_EXPAND_SIDE_EFFECTS((params[VIndex] = input_iter.template data<BIndex>()));
            returned_array::call(
