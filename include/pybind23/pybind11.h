/*
    pybind23/pybind23.h: Main header file of the C++11 python
...
// See PR #5448. This warning suppression is needed for the PYBIND23_OVERRIDE macro family.
...
PYBIND23_WARNING_DISABLE_GCC("-Wnoexcept-type")
#endif
...
inline std::string generate_function_signature(const char *type_caster_name_field,
    ...
    for (const auto *pc = type_caster_name_field; *pc != '\0'; ++pc) {
        ...
        if (c == '{') {
            ...
            if (!t) {
                pybind23_fail("Internal error while parsing type signature (1)");
            }
...
inline std::string generate_type_signature() {
    ...
    PYBIND23_DESCR_CONSTEXPR auto descr_types = decltype(caster_name_field)::types();
...
#if defined(_MSC_VER)
#    define PYBIND23_COMPAT_STRDUP _strdup
#else
#    define PYBIND23_COMPAT_STRDUP strdup
#endif
...
protected:
    ...
    PYBIND23_NOINLINE unique_function_record make_function_record() {
    ...
    void initialize(Func &&f, Return (*)(Args...), const Extra &...extra) {
        ...
        struct capture {
            ...
            static capture *from_data(void **data) {
                return PYBIND23_STD_LAUNDER(reinterpret_cast<capture *>(data));
            }
        };
        ...
        if (sizeof(capture) <= sizeof(rec->data)) {
            ...
            PYBIND23_WARNING_DISABLE_GCC("-Wplacement-new")
#endif

            new (capture::from_data(rec->data)) capture{std::forward<Func>(f)};

#if !PYBIND23_HAS_STD_LAUNDER
            PYBIND23_WARNING_DISABLE_GCC("-Wstrict-aliasing")
#endif
        ...
        rec->impl = [](function_call &call) -> handle {
            ...
            if (!args_converter.load_args(call)) {
                return PYBIND23_TRY_NEXT_OVERLOAD;
            }
        ...
        PYBIND23_DESCR_CONSTEXPR auto types = decltype(signature)::types();
        ...
        PYBIND23_ENSURE_PRECONDITION_FOR_FUNCTIONAL_H_PERFORMANCE_OPTIMIZATIONS(
    ...
    public:
        ...
        char *operator()(const char *s) {
            auto *t = PYBIND23_COMPAT_STRDUP(s);
            strings.push_back(t);
            return t;
        }
    ...
    void initialize_generic(unique_function_record &&unique_rec,
...
#if defined(PYBIND23_DETAILED_ERROR_MESSAGES) && !defined(PYBIND23_DISABLE_NEW_STYLE_INIT_WARNING)
        if (rec->is_constructor && !rec->is_new_style_constructor) {
            const auto class_name
                = detail::get_fully_qualified_tp_name((PyTypeObject *) rec->scope.ptr());
            const auto func_name = std::string(rec->name);
            PyErr_WarnEx(PyExc_FutureWarning,
                         ("pybind23-bound class '" + class_name
                          + "' is using an old-style "
                            "placement-new '"
                          + func_name
                          + "' which has been deprecated. See "
                            "the upgrade guide in pybind23's docs. This message is only visible "
        ...
        if (arg_index != args - rec->has_args - rec->has_kwargs || types[type_index] != nullptr) {
            pybind23_fail("Internal error while parsing type signature (2)");
        }
        ...
        if (rec->sibling && PYBIND23_INSTANCE_METHOD_CHECK(rec->sibling.ptr())) {
            rec->sibling = PYBIND23_INSTANCE_METHOD_GET_FUNCTION(rec->sibling.ptr());
        }
        ...
        if (rec->sibling) {
            ...
            // that we are intentionally replacing
                ...
                pybind23_fail("Cannot overload existing non-function object \""
        ...
        if (!chain) {
            ...
            if (!m_ptr) {
                pybind23_fail("cpp_function::cpp_function(): Could not allocate function object");
            }
        } else {
            ...
            if (chain->is_method != rec->is_method) {
                pybind23_fail(
                    ...
                    "#define PYBIND23_DETAILED_ERROR_MESSAGES or compile in debug mode for more "
                    "details"
#else
                    "error while attempting to bind "
                    + std::string(rec->is_method ? "instance" : "static") + " method "
                    + std::string(pybind23::str(rec->scope.attr("__name__"))) + "."
        ...
        if (chain && options::show_function_signatures()
            && std::strcmp(rec->name, "_pybind23_conduit_v1_") != 0) {
        ...
        auto *doc = signatures.empty() ? nullptr : PYBIND23_COMPAT_STRDUP(signatures.c_str());
        std::free(const_cast<char *>(PYBIND23_PYCFUNCTION_GET_DOC(func)));
        PYBIND23_PYCFUNCTION_SET_DOC(func, doc);

        if (rec->is_method) {
            m_ptr = PYBIND23_INSTANCE_METHOD_NEW(m_ptr, rec->scope.ptr());
            if (!m_ptr) {
                pybind23_fail(
    ...
    /// When a cpp_function is GCed, release any memory allocated by pybind23
...
// See https://github.com/python/cpython/pull/22670
    ...
    /// Main dispatch logic for calls to functions bound using pybind23
    static PyObject *dispatcher(PyObject *self, PyObject *args_in, PyObject *kwargs_in) {
        ...
        handle parent = n_args_in > 0 ? PyTuple_GET_ITEM(args_in, 0) : nullptr,
               result = PYBIND23_TRY_NEXT_OVERLOAD;
        ...
        try {
            ...
            for (; current_overload != nullptr; current_overload = current_overload->next) {
                ...
                /* For each overload:
                   ...
                   a result other than PYBIND23_TRY_NEXT_OVERLOAD.
                 */
...
// in `call.args` all along.
                ...
                if (call.args.size() != func.nargs || call.args_convert.size() != func.nargs) {
                    pybind23_fail("Internal error: function call dispatcher inserted wrong number "
                                  "of arguments!");
                }
#endif
                ...
                if (result.ptr() != PYBIND23_TRY_NEXT_OVERLOAD) {
                    break;
                }
            ...
            if (overloaded && !second_pass.empty() && result.ptr() == PYBIND23_TRY_NEXT_OVERLOAD) {
        ...
        auto append_note_if_missing_header_is_suspected = [](std::string &msg) {
            if (msg.find("std::") != std::string::npos) {
                msg += "\n\n"
                       "Did you forget to `#include <pybind23/stl.h>`? Or <pybind23/complex.h>,\n"
                       "<pybind23/functional.h>, <pybind23/chrono.h>, etc. Some automatic\n"
                       "conversions are optional and require extra headers to be included\n"
                       "when compiling your pybind23 module.";
            }
        };
            ...
            if (kwargs_in) {
                ...
                if (!kwargs.empty()) {
                    ...
                    for (const auto &kwarg : kwargs) {
                        ...
                        msg += pybind23::str("{}=").format(kwarg.first);
                        try {
                            msg += pybind23::repr(kwarg.second);
                        } catch (const error_already_set &) {
                            msg += "<repr raised Error>";
                        }
                    }
...
PYBIND23_NAMESPACE_BEGIN(function_record_PyTypeObject_methods)
...
PYBIND23_NAMESPACE_END(function_record_PyTypeObject_methods)
...
*/
inline PyObject *get_cached_module(pybind23::str const &nameobj) {
    dict state = detail::get_python_state_dict();
    if (!state.contains("__pybind23_module_cache")) {
        return nullptr;
    }
    dict cache = state["__pybind23_module_cache"];
...
*/
inline void cache_completed_module(pybind23::object const &mod) {
    dict state = detail::get_python_state_dict();
    if (!state.contains("__pybind23_module_cache")) {
        state["__pybind23_module_cache"] = dict();
    }
    state["__pybind23_module_cache"][mod.attr("__spec__").attr("name")] = mod;
}
...
public:
    PYBIND23_OBJECT_DEFAULT(module_, object, PyModule_Check)

    /// Create a new top-level Python module with the given name and docstring
    PYBIND23_DEPRECATED("Use PYBIND23_MODULE or module_::create_extension_module instead")
    ...
    /** \rst
        ...
        .. code-block:: cpp
            ...
            py::module_ m("example", "pybind23 example plugin");
    ...
    module_ def_submodule(const char *name, const char *doc = nullptr) {
        ...
        if (doc && options::show_user_defined_docstrings()) {
            result.attr("__doc__") = pybind23::str(doc);
        }
    ...
    /** \rst
        ...
        ``overwrite`` should almost always be false: attempting to overwrite objects that pybind23
        has established will, in most cases, break things.
    \endrst */
    PYBIND23_NOINLINE void add_object(const char *name, handle obj, bool overwrite = false) {
    ...
    static module_ create_extension_module(const char *name,
        ...
        if (m == nullptr) {
            ...
            pybind23_fail("Internal error in module_::create_extension_module()");
        }
...
public:
    PYBIND23_OBJECT_DEFAULT(generic_type, object, PyType_Check)
protected:
    void initialize(const type_record &rec) {
        if (rec.scope && hasattr(rec.scope, "__dict__")
            && rec.scope.attr("__dict__").contains(rec.name)) {
            pybind23_fail("generic_type: cannot initialize type \"" + std::string(rec.name)
                          + "\": an object with that name is already defined");
        }

        if ((rec.module_local ? get_local_type_info(*rec.type) : get_global_type_info(*rec.type))
            != nullptr) {
            pybind23_fail("generic_type: type \"" + std::string(rec.name)
        ...
        with_internals([&](internals &internals) {
            ...
            PYBIND23_WARNING_DISABLE_GCC("-Warray-bounds")
            PYBIND23_WARNING_DISABLE_GCC("-Wstringop-overread")
#endif
        ...
        if (rec.module_local) {
            ...
            setattr(m_ptr, PYBIND23_MODULE_LOCAL_ID, capsule(tinfo));
        }
    }
    ...
    void install_buffer_funcs(buffer_info *(*get_buffer)(PyObject *, void *),
        ...
        if (!type->ht_type.tp_as_buffer) {
            pybind23_fail("To be able to register buffer protocol support for the type '"
                          + get_fully_qualified_tp_name(tinfo->type)
                          + "' the associated class<>(..) invocation must "
                            "include the pybind23::buffer_protocol() annotation!");
        }
...
// Note that property_cpp_function is intentionally in the main pybind23 namespace,
...
#ifdef PYBIND23_RUN_TESTING_WITH_SMART_HOLDER_AS_DEFAULT_BUT_NEVER_USE_IN_PRODUCTION_PLEASE
// NOTE: THIS IS MEANT FOR STRESS-TESTING OR TRIAGING ONLY!
//       Running the pybind23 unit tests with smart_holder as the default holder is to ensure
...
public:
    ...
    static_assert(!has_alias || !detail::is_smart_holder<holder_type>::value
                  ...
                  " pybind23::trampoline_self_life_support if used in combination with"
                  " pybind23::smart_holder");
#endif
    static_assert(!has_alias || detail::is_smart_holder<holder_type>::value
                      || !std::is_base_of<trampoline_self_life_support, type_alias>::value,
                  "pybind23::trampoline_self_life_support is a smart_holder feature, therefore"
                  " an alias class (aka trampoline) should inherit from"
                  " pybind23::trampoline_self_life_support only if used in combination with"
                  " pybind23::smart_holder");

    PYBIND23_OBJECT(class_, generic_type, PyType_Check)
    ...
    class_(handle scope, const char *name, const Extra &...extra) {
        ...
        PYBIND23_EXPAND_SIDE_EFFECTS(add_base<options>(record));
        ...
        def("_pybind23_conduit_v1_", cpp_conduit_method);
    }
    ...
    class_ &def(const detail::initimpl::constructor<Args...> &init, const Extra &...extra) {
        PYBIND23_WORKAROUND_INCORRECT_MSVC_C4100(init);
    ...
    class_ &def_property_static(const char *name,
        ...
        if (rec_fget) {
            ...
            if (rec_fget->doc && rec_fget->doc != doc_prev) {
                std::free(doc_prev);
                rec_fget->doc = PYBIND23_COMPAT_STRDUP(rec_fget->doc);
            }
        }
        if (rec_fset) {
            ...
            if (rec_fset->doc && rec_fset->doc != doc_prev) {
                std::free(doc_prev);
                rec_fset->doc = PYBIND23_COMPAT_STRDUP(rec_fset->doc);
            }
            if (!rec_active) {
                rec_active = rec_fset;
            }
...
private:
    ...
    // calls, those calls are likely to generate a new exception, and pybind23 will then
...
struct enum_base {
    ...
    PYBIND23_NOINLINE void init(bool is_arithmetic, bool is_convertible) {
        ...
        m_base.attr("__str__") = cpp_function(
            [](handle arg) -> str {
                ...
                return pybind23::str("{}.{}").format(std::move(type_name), enum_name(arg));
            },
            name("__str__"),
            is_method(m_base),
            pos_only());
            ...
            m_base.attr("__doc__") = static_property(
                cpp_function(
                    [](handle arg) -> std::string {
                        ...
                        docstring += "Members:";
                        for (auto kv : entries) {
                            auto key = std::string(pybind23::str(kv.first));
                            auto comment = kv.second[int_(1)];
                            docstring += "\n\n  ";
                            docstring += key;
                            if (!comment.is_none()) {
                                docstring += " : ";
                                docstring += pybind23::str(comment).cast<std::string>();
                            }
                        }
                        return docstring;
                    },
...
#define PYBIND23_ENUM_OP_STRICT(op, expr, strict_behavior)                                        \
...
#define PYBIND23_ENUM_OP_CONV(op, expr)                                                           \
    m_base.attr(op) = cpp_function(                                                               \
...
#define PYBIND23_ENUM_OP_CONV_LHS(op, expr)                                                       \
    m_base.attr(op) = cpp_function(                                                               \
        ...
        if (is_convertible) {
            PYBIND23_ENUM_OP_CONV_LHS("__eq__", !b.is_none() && a.equal(b));
            PYBIND23_ENUM_OP_CONV_LHS("__ne__", b.is_none() || !a.equal(b));

            if (is_arithmetic) {
                PYBIND23_ENUM_OP_CONV("__lt__", a < b);
                PYBIND23_ENUM_OP_CONV("__gt__", a > b);
                PYBIND23_ENUM_OP_CONV("__le__", a <= b);
                PYBIND23_ENUM_OP_CONV("__ge__", a >= b);
                PYBIND23_ENUM_OP_CONV("__and__", a & b);
                PYBIND23_ENUM_OP_CONV("__rand__", a & b);
                PYBIND23_ENUM_OP_CONV("__or__", a | b);
                PYBIND23_ENUM_OP_CONV("__ror__", a | b);
                PYBIND23_ENUM_OP_CONV("__xor__", a ^ b);
                PYBIND23_ENUM_OP_CONV("__rxor__", a ^ b);
            ...
            PYBIND23_ENUM_OP_STRICT("__eq__", int_(a).equal(int_(b)), return false);
            PYBIND23_ENUM_OP_STRICT("__ne__", !int_(a).equal(int_(b)), return true);

            if (is_arithmetic) {
#define PYBIND23_THROW throw type_error("Expected an enumeration of matching type!");
                PYBIND23_ENUM_OP_STRICT("__lt__", int_(a) < int_(b), PYBIND23_THROW);
                PYBIND23_ENUM_OP_STRICT("__gt__", int_(a) > int_(b), PYBIND23_THROW);
                PYBIND23_ENUM_OP_STRICT("__le__", int_(a) <= int_(b), PYBIND23_THROW);
                PYBIND23_ENUM_OP_STRICT("__ge__", int_(a) >= int_(b), PYBIND23_THROW);
#undef PYBIND23_THROW
            }
        }

#undef PYBIND23_ENUM_OP_CONV_LHS
#undef PYBIND23_ENUM_OP_CONV
#undef PYBIND23_ENUM_OP_STRICT
    ...
    PYBIND23_NOINLINE void value(char const *name_, object value, const char *doc = nullptr) {
        ...
        entries[name] = pybind23::make_tuple(value, doc);
        m_base.attr(std::move(name)) = std::move(value);
    }

    PYBIND23_NOINLINE void export_values() {
        dict entries = m_base.attr("__entries");
        for (auto kv : entries) {
...
public:
    ...
    enum_(const handle &scope, const char *name, const Extra &...extra)
        : class_<Type>(scope, name, extra...), m_base(*this, scope) {
        {
            if (detail::global_internals_native_enum_type_map_contains(
                ...
                pybind23_fail("pybind23::enum_ \"" + std::string(name)
                              + "\" is already registered as a pybind23::native_enum!");
            }
        }
    ...
    enum_ &value(char const *name, Type value, const char *doc = nullptr) {
        m_base.value(name, pybind23::cast(value, return_value_policy::copy), doc);
        return *this;
    }
...
PYBIND23_NOINLINE void keep_alive_impl(handle nurse, handle patient) {
    if (!nurse || !patient) {
        pybind23_fail("Could not activate keep_alive!");
    }
    ...
    if (res.second) {
        ...
        weakref((PyObject *) type, cpp_function([type](handle wr) {
                    with_internals([type](internals &internals) {
                        ...
                        // TODO consolidate the erasure code in pybind23_meta_dealloc() in class.h
...
iterator make_iterator_impl(Iterator first, Sentinel last, Extra &&...extra) {
    ...
    if (!detail::get_type_info(typeid(state), false)) {
        class_<state>(handle(), "iterator", pybind23::module_local())
            .def(
...
void implicitly_convertible() {
    ...
    if (auto *tinfo = detail::get_type_info(typeid(OutputType))) {
        ...
        pybind23_fail("implicitly_convertible: Unable to find type " + type_id<OutputType>());
    }
}
...
public:
    ...
    exception(handle scope, const char *name, handle base = PyExc_Exception) {
        ...
        if (hasattr(scope, "__dict__") && scope.attr("__dict__").contains(name)) {
            pybind23_fail("Error during initialization: multiple incompatible "
    ...
    PYBIND23_DEPRECATED("Please use py::set_error() instead "
                        "(https://github.com/pybind/pybind23/pull/4772)")
...
register_exception_impl(handle scope, const char *name, handle base, bool isLocal) {
    PYBIND23_CONSTINIT static gil_safe_call_once_and_store<exception<CppException>> exc_storage;
...
PYBIND23_NAMESPACE_BEGIN(detail)
PYBIND23_NOINLINE void print(const tuple &args, const dict &kwargs) {
...
#define PYBIND23_OVERRIDE_IMPL(ret_type, cname, name, ...)                                        \
    do {                                                                                          \
        pybind23::gil_scoped_acquire gil;                                                         \
        pybind23::function override                                                               \
            = pybind23::get_override(static_cast<const cname *>(this), name);                     \
        if (override) {                                                                           \
            auto o = override(__VA_ARGS__);                                                       \
            PYBIND23_WARNING_PUSH                                                                 \
            PYBIND23_WARNING_DISABLE_MSVC(4127)                                                   \
            if (pybind23::detail::cast_is_temporary_value_reference<ret_type>::value              \
                && !pybind23::detail::is_same_ignoring_cvref<ret_type, PyObject *>::value) {      \
                static pybind23::detail::override_caster_t<ret_type> caster;                      \
                return pybind23::detail::cast_ref<ret_type>(std::move(o), caster);                \
            }                                                                                     \
            PYBIND23_WARNING_POP                                                                  \
            return pybind23::detail::cast_safe<ret_type>(std::move(o));                           \
        }                                                                                         \
    } while (false)
...
\endrst */
#define PYBIND23_OVERRIDE_NAME(ret_type, cname, name, fn, ...)                                    \
    do {                                                                                          \
        PYBIND23_OVERRIDE_IMPL(PYBIND23_TYPE(ret_type), PYBIND23_TYPE(cname), name, __VA_ARGS__); \
        return cname::fn(__VA_ARGS__);                                                            \
    } while (false)

/** \rst
    Macro for pure virtual functions, this function is identical to
    :c:macro:`PYBIND23_OVERRIDE_NAME`, except that it throws if no override can be found.
\endrst */
#define PYBIND23_OVERRIDE_PURE_NAME(ret_type, cname, name, fn, ...)                               \
    do {                                                                                          \
        PYBIND23_OVERRIDE_IMPL(PYBIND23_TYPE(ret_type), PYBIND23_TYPE(cname), name, __VA_ARGS__); \
        pybind23::pybind23_fail(                                                                  \
            "Tried to call pure virtual function \"" PYBIND23_STRINGIFY(cname) "::" name "\"");   \
    } while (false)
...
\endrst */
#define PYBIND23_OVERRIDE(ret_type, cname, fn, ...)                                               \
    PYBIND23_OVERRIDE_NAME(PYBIND23_TYPE(ret_type), PYBIND23_TYPE(cname), #fn, fn, __VA_ARGS__)

/** \rst
    Macro for pure virtual functions, this function is identical to :c:macro:`PYBIND23_OVERRIDE`,
    except that it throws if no override can be found.
\endrst */
#define PYBIND23_OVERRIDE_PURE(ret_type, cname, fn, ...)                                          \
    PYBIND23_OVERRIDE_PURE_NAME(                                                                  \
        PYBIND23_TYPE(ret_type), PYBIND23_TYPE(cname), #fn, fn, __VA_ARGS__)

// Deprecated versions

PYBIND23_DEPRECATED("get_type_overload has been deprecated")
inline function
...
#define PYBIND23_OVERLOAD_INT(ret_type, cname, name, ...)                                         \
    PYBIND23_OVERRIDE_IMPL(PYBIND23_TYPE(ret_type), PYBIND23_TYPE(cname), name, __VA_ARGS__)
#define PYBIND23_OVERLOAD_NAME(ret_type, cname, name, fn, ...)                                    \
    PYBIND23_OVERRIDE_NAME(PYBIND23_TYPE(ret_type), PYBIND23_TYPE(cname), name, fn, __VA_ARGS__)
#define PYBIND23_OVERLOAD_PURE_NAME(ret_type, cname, name, fn, ...)                               \
    PYBIND23_OVERRIDE_PURE_NAME(                                                                  \
        PYBIND23_TYPE(ret_type), PYBIND23_TYPE(cname), name, fn, __VA_ARGS__);
#define PYBIND23_OVERLOAD(ret_type, cname, fn, ...)                                               \
    PYBIND23_OVERRIDE(PYBIND23_TYPE(ret_type), PYBIND23_TYPE(cname), fn, __VA_ARGS__)
#define PYBIND23_OVERLOAD_PURE(ret_type, cname, fn, ...)                                          \
    PYBIND23_OVERRIDE_PURE(PYBIND23_TYPE(ret_type), PYBIND23_TYPE(cname), fn, __VA_ARGS__);
