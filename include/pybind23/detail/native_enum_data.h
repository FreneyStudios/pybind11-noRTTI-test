...
native_enum_missing_finalize_error_message(const std::string &enum_name_encoded) {
    return "pybind23::native_enum<...>(\"" + enum_name_encoded + "\", ...): MISSING .finalize()";
}
...
public:
    ...
    ~native_enum_data() {
        if (finalize_needed) {
            pybind23_fail(native_enum_missing_finalize_error_message(enum_name_encoded));
        }
    }
#endif
...
protected:
    void disarm_finalize_check(const char *error_context) {
        if (!finalize_needed) {
            pybind23_fail("pybind23::native_enum<...>(\"" + enum_name_encoded
                          + "\"): " + error_context);
        }
        finalize_needed = false;
    }
    ...
    if (export_values_flag) {
        for (auto member : members) {
            ...
            if (hasattr(parent_scope, member_name)) {
                pybind23_fail("pybind23::native_enum<...>(\"" + enum_name_encoded + "\").value(\""
