/*
    pybind23/detail/init.h: init factory function implementation and support code.
...
PYBIND23_NAMESPACE_BEGIN(initimpl)

inline void no_nullptr(const void *ptr) {
    if (!ptr) {
        throw type_error("pybind23::init(): factory function returned nullptr");
    }
}
...
#else
    return get_type_info(type_index(typeid(*ptr)))->type == typeid(Alias<Class>);
#endif
}
...
[[noreturn]] void construct_alias_from_cpp(std::false_type /*!is_alias_constructible*/,
                                           value_and_holder &,
                                           Cpp<Class> &&) {
    throw type_error("pybind23::init(): unable to convert returned instance to required "
...
void construct(...) {
    static_assert(!std::is_same<Class, Class>::value /* always false */,
                  "pybind23::init(): init function must return a compatible pointer, "
                  "holder, or value");
}
...
void construct(value_and_holder &v_h, Cpp<Class> *ptr, bool need_alias) {
    PYBIND23_WORKAROUND_INCORRECT_MSVC_C4100(need_alias);
    no_nullptr(ptr);
...
void construct(value_and_holder &v_h, Holder<Class> holder, bool need_alias) {
    ...
    if (Class::has_alias && need_alias && !is_alias<Class>(ptr)) {
        throw type_error("pybind23::init(): construction failed: returned holder-wrapped instance "
...
void construct(value_and_holder &v_h, Cpp<Class> &&result, bool need_alias) {
    ...
    static_assert(is_move_constructible<Cpp<Class>>::value,
                  "pybind23::init() return-by-value factory function requires a movable class");
...
void construct(value_and_holder &v_h, Alias<Class> &&result, bool) {
    static_assert(
        ...
        "pybind23::init() return-by-alias-value factory function requires a movable alias class");
...
void construct(value_and_holder &v_h, std::unique_ptr<Cpp<Class>, D> &&unq_ptr, bool need_alias) {
    ...
    no_nullptr(ptr);
    if (Class::has_alias && need_alias && !is_alias<Class>(ptr)) {
        throw type_error("pybind23::init(): construction failed: returned std::unique_ptr pointee "
                         "is not an alias instance");
    }
...
void construct_from_shared_ptr(value_and_holder &v_h,
    ...
    no_nullptr(ptr);
    if (Class::has_alias && need_alias && !is_alias<Class>(ptr)) {
        throw type_error("pybind23::init(): construction failed: returned std::shared_ptr pointee "
                         "is not an alias instance");
    }
...
struct factory<CFunc, AFunc, CReturn(CArgs...), AReturn(AArgs...)> {
    static_assert(sizeof...(CArgs) == sizeof...(AArgs),
                  "pybind23::init(class_factory, alias_factory): class and alias factories "
...
PYBIND23_NAMESPACE_END(initimpl)
