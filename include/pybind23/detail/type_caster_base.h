/*
    pybind23/detail/type_caster_base.h (originally first part of pybind23/cast.h)
...
#include <pybind23/gil.h>
#include <pybind23/pytypes.h>
#include <pybind23/trampoline_self_life_support.h>
...
public:
    ...
    ~loader_life_support() {
        ...
        if (stack_top.get() != this) {
            pybind23_fail("loader_life_support: internal error");
        }
        stack_top = parent;
    ...
    /// This can only be used inside a pybind23-bound function, either by `argument_loader`
    /// at argument preparation time or by `py::cast()` at execution time.
    PYBIND23_NOINLINE static void add_patient(handle h) {
        loader_life_support *frame = get_internals().loader_life_support_tls.get();
        if (!frame) {
            // NOTE: It would be nice to include the stack frames here, as this indicates
            // use of pybind23::cast<> outside the normal call framework, finding such
...
PYBIND23_NOINLINE void all_type_info_populate(PyTypeObject *t, std::vector<type_info *> &bases) {
    assert(bases.empty());
    ...
    for (size_t i = 0; i < check.size(); i++) {
        ...
        if (it != type_dict.end()) {
            ...
            for (auto *tinfo : it->second) {
                ...
                // having a large number of immediate pybind23-registered types seems fairly
...
/**
 * Gets a single pybind23 type info for a python type.  Returns nullptr if neither the type nor any
 * ancestors are pybind23-registered.  Throws an exception if there are multiple bases--use
 * `all_type_info` instead if you want to support multiple bases.
 */
PYBIND23_NOINLINE detail::type_info *get_type_info(PyTypeObject *type) {
    ...
    if (bases.size() > 1) {
        pybind23_fail(
            "pybind23::detail::get_type_info: type has multiple pybind23-registered bases");
    }
    return bases.front();
}

inline detail::type_info *get_local_type_info(const type_index &tp) {
...
inline detail::type_info *get_global_type_info(const type_index &tp) {
...
/// nullptr.
PYBIND23_NOINLINE detail::type_info *get_type_info(const type_index &tp,
    ...
    if (throw_if_missing) {
        ...
        pybind23_fail("pybind23::detail::get_type_info: unable to find type info for \""
                      + std::move(tname) + '"');
    }
    return nullptr;
}

PYBIND23_NOINLINE handle get_type_handle(const std::type_info &tp, bool throw_if_missing) {
    detail::type_info *type_info = get_type_info(type_index(tp), throw_if_missing);
...
PYBIND23_NOINLINE handle find_registered_python_instance(void *src,
    ...
    return with_instance_map(src, [&](instance_map &instances) {
        ...
        for (auto it_i = it_instances.first; it_i != it_instances.second; ++it_i) {
            for (auto *instance_type : detail::all_type_info(Py_TYPE(it_i->second))) {
                if (instance_type && *instance_type->cpptype == *tinfo->cpptype) {
 ...
 * if the given type (or ValueType, if omitted) is not a pybind23 base of the given instance.  If
 ...
 * instance.
 */
PYBIND23_NOINLINE value_and_holder
instance::get_value_and_holder(const type_info *find_type /*= nullptr default in common.h*/,
    ...
    pybind23_fail("pybind23::detail::instance::get_value_and_holder: `"
                  + get_fully_qualified_tp_name(find_type->type)
                  + "' is not a pybind23 base of the given `"
                  + get_fully_qualified_tp_name(Py_TYPE(this)) + "' instance");
#else
    pybind23_fail(
        "pybind23::detail::instance::get_value_and_holder: "
        "type is not a pybind23 base of the given instance "
        "(#define PYBIND23_DETAILED_ERROR_MESSAGES or compile in debug mode for type details)");
#endif
}

PYBIND23_NOINLINE void instance::allocate_layout() {
    ...
    if (n_types == 0) {
        pybind23_fail(
            "instance allocation failed: new instance has no pybind23-registered base types");
    }
...
PYBIND23_NOINLINE void instance::deallocate_layout() {
    if (!simple_layout) {
        PyMem_Free(reinterpret_cast<void *>(nonsimple.values_and_holders));
    }
}

PYBIND23_NOINLINE bool isinstance_generic(handle obj, const std::type_info &tp) {
...
PYBIND23_NOINLINE handle get_object_handle(const void *ptr, const detail::type_info *type) {
...
PYBIND23_WARNING_DISABLE_GCC("-Wredundant-decls")

// PYBIND23:REMINDER: Needs refactoring of existing pybind23 code.
inline bool deregister_instance(instance *self, void *valptr, const type_info *tinfo);

PYBIND23_WARNING_POP

PYBIND23_NAMESPACE_BEGIN(smart_holder_type_caster_support)
        ...
        if (self_life_support != nullptr) {
            ...
            if (v_h.inst != nullptr && v_h.vh != nullptr) {
                ...
                if (!holder.is_disowned) {
                    pybind23_fail("smart_holder_from_unique_ptr: unexpected "
...
handle smart_holder_from_shared_ptr(const std::shared_ptr<T> &src,
    ...
    const detail::type_info *tinfo = st.second;
    if (handle existing_inst = find_registered_python_instance(src_raw_void_ptr, tinfo)) {
        // PYBIND23:REMINDER: MISSING: Enforcement of consistency with existing smart_holder.
        // PYBIND23:REMINDER: MISSING: keep_alive.
        return existing_inst;
    }
...
inline std::unique_ptr<T, D> unique_with_deleter(T *raw_ptr, std::unique_ptr<D> &&deleter) {
    if (deleter == nullptr) {
        pybind23_fail("smart_holder_type_casters: deleter is not default constructible and no"
        ...
        if (python_instance_is_alias && !force_potentially_slicing_shared_ptr) {
            ...
            if (sptsls_ptr != nullptr) {
                ...
                if (reinterpret_cast<PyObject *>(loaded_v_h.inst) == sptsls_ptr->self) {
                    pybind23_fail("smart_holder_type_caster_support load_as_shared_ptr failure: "
            ...
            if (hld.vptr_is_external_shared_ptr) {
                pybind23_fail("smart_holder_type_casters load_as_shared_ptr failure: not "
...
PYBIND23_NAMESPACE_END(smart_holder_type_caster_support)

class type_caster_generic {
public:
    PYBIND23_NOINLINE explicit type_caster_generic(const std::type_info &type_info)
        : typeinfo(get_type_info(type_index(type_info))), cpptype(&type_info) {}
    ...
    PYBIND23_NOINLINE static handle cast(const void *_src,
                                         return_value_policy policy,
                                         handle parent,
                                         const detail::type_info *tinfo,
            ...
            case return_value_policy::copy:
                if (copy_constructor) {
                    ...
                    throw cast_error("return_value_policy = copy, but type is "
                                     "non-copyable! (#define PYBIND23_DETAILED_ERROR_MESSAGES or "
            ...
            case return_value_policy::move:
                if (move_constructor) {
                    ...
                    throw cast_error("return_value_policy = move, but type is neither "
                                     ...
                                     "(#define PYBIND23_DETAILED_ERROR_MESSAGES or compile in "
    ...
    PYBIND23_NOINLINE static void *local_load(PyObject *src, const type_info *ti) {
    ...
    PYBIND23_NOINLINE bool try_load_foreign_module_local(handle src) {
        constexpr auto *local_key = PYBIND23_MODULE_LOCAL_ID;
        ...
        if (foreign_typeinfo->module_local_load == &local_load
            || (cpptype && *cpptype != *foreign_typeinfo->cpptype)) {
            return false;
        }
    ...
    PYBIND23_NOINLINE bool load_impl(handle src, bool convert) {
        if (!src) {
            return false;
        }
        if (!typeinfo) {
        ...
        if (PyType_IsSubtype(srctype, typeinfo->type)) {
            ...
            // one simple (no MI) pybind23 class, or is an exact match, so the C++ instance is of
    ...
    PYBIND23_NOINLINE static std::pair<const void *, const type_info *>
    src_and_type(const void *src,
                 const std::type_info &cast_type,
                 const std::type_info *rtti_type = nullptr) {
        if (auto *tpi = get_type_info(type_index(cast_type))) {
...
inline object cpp_conduit_method(handle self,
                                 const bytes &pybind23_platform_abi_id,
...
#endif
    if (cpp_str(pybind23_platform_abi_id) != PYBIND23_PLATFORM_ABI_ID) {
        return none();
    }
    ...
    const auto *cpp_type_info_ptr = cpp_type_info_capsule.get_pointer<const std::type_info>();
    type_caster_generic caster(get_type_info(type_index(*cpp_type_info_ptr)));
    if (!caster.load(self, false)) {
        return none();
    }
    return capsule(caster.value, cpp_type_info_ptr->name());
}
 ...
 *
 * These operator are automatically provided when using the PYBIND23_TYPE_CASTER macro.
 */
template <typename T>
...
// registered with pybind23, and this Animal is in fact a Dog).
//
...
public:
    ...
    static std::pair<const void *, const type_info *> src_and_type(const itype *src) {
        ...
        if (instance_type && *instance_type != cast_type) {
            // This is a base pointer to a derived type. If the derived type is registered
            // with pybind23, we want to make the full derived object available.
            ...
            if (const auto *tpi = get_type_info(type_index(*instance_type))) {
                return {vsrc, tpi};
            }
        }
...
#else
        const auto *tpi = get_type_info(type_index(typeid(itype)));
        return {src, tpi};
#endif
    }
...
PYBIND23_NOINLINE std::string type_info_description(const std::type_info &ti) {
    if (auto *type_data = get_type_info(type_index(ti))) {
