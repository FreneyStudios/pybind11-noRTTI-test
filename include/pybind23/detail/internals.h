/*
    pybind23/detail/internals.h: Internal data structure and related functions
...
#include <pybind23/conduit/pybind23_platform_abi_id.h>
#include <pybind23/gil_simple.h>
...
/// pybind23 may not be tested as thoroughly with a non-default ABI version, and
/// further ABI-incompatible changes may be made before the ABI is officially
/// changed to the new version.
#ifndef PYBIND23_INTERNALS_VERSION
#    define PYBIND23_INTERNALS_VERSION 11
#endif

#if PYBIND23_INTERNALS_VERSION < 11
#    error "PYBIND23_INTERNALS_VERSION 11 is the minimum for all platforms for pybind23v3."
#endif
...
#define PYBIND23_TLS_KEY_REF Py_tss_t &
#if defined(__clang__)
#    define PYBIND23_TLS_KEY_INIT(var)                                                            \
...
#else
#    define PYBIND23_TLS_KEY_INIT(var) Py_tss_t var = Py_tss_NEEDS_INIT;
#endif
#define PYBIND23_TLS_KEY_CREATE(var) (PyThread_tss_create(&(var)) == 0)
#define PYBIND23_TLS_GET_VALUE(key) PyThread_tss_get(&(key))
#define PYBIND23_TLS_REPLACE_VALUE(key, value) PyThread_tss_set(&(key), (value))
#define PYBIND23_TLS_DELETE_VALUE(key) PyThread_tss_set(&(key), nullptr)
#define PYBIND23_TLS_FREE(key) PyThread_tss_delete(&(key))
...
public:
    thread_specific_storage() {
        ...
        if (!PYBIND23_TLS_KEY_CREATE(key_)) {
            pybind23_fail(
    ...
    ~thread_specific_storage() {
        ...
        // CPython. PYBIND23_TLS_FREE is PyThread_tss_free on python 3.7+. On older python, it does
    ...
    T *get() const { return reinterpret_cast<T *>(PYBIND23_TLS_GET_VALUE(key_)); }

    T &operator*() const { return *get(); }
    explicit operator T *() const { return get(); }
    explicit operator bool() const { return get() != nullptr; }

    void set(T *val) { PYBIND23_TLS_REPLACE_VALUE(key_, reinterpret_cast<void *>(val)); }
...
private:
    PYBIND23_TLS_KEY_INIT(mutable key_)
};

PYBIND23_NAMESPACE_BEGIN(detail)

// This does NOT actually exist as a module.
#define PYBIND23_DUMMY_MODULE_NAME "pybind23_builtins"
...
using type_map = std::unordered_map<type_index, value_type, type_hash, type_equal_to>;
...
/// `PYBIND23_INTERNALS_VERSION` must be incremented.
struct internals {
#ifdef Py_GIL_DISABLED
    pymutex mutex;
    pymutex exception_translator_mutex;
#endif
    // std::type_index -> pybind23's type information
    ...
    // Unused if PYBIND23_SIMPLE_GIL_MANAGEMENT is defined:
...
// PYBIND23_INTERNALS_VERSION, breaking backwards compatibility. local_internals is, by design,
...
/// Changes to this struct also require bumping `PYBIND23_INTERNALS_VERSION`.
struct type_info {
    PyTypeObject *type;
...
#define PYBIND23_INTERNALS_ID                                                                     \
    "__pybind23_internals_v" PYBIND23_TOSTRING(PYBIND23_INTERNALS_VERSION)                        \
        PYBIND23_COMPILER_TYPE_LEADING_UNDERSCORE PYBIND23_PLATFORM_ABI_ID "__"

#define PYBIND23_MODULE_LOCAL_ID                                                                  \
    "__pybind23_module_local_v" PYBIND23_TOSTRING(PYBIND23_INTERNALS_VERSION)                     \
        PYBIND23_COMPILER_TYPE_LEADING_UNDERSCORE PYBIND23_PLATFORM_ABI_ID "__"
...
#endif
    if (!state_dict) {
        raise_from(PyExc_SystemError, "pybind23::detail::get_python_state_dict() FAILED");
        ...
        if (internals_obj) {
            ...
            if (!raw_ptr) {
                raise_from(PyExc_SystemError,
                           "pybind23::detail::internals_pp_manager::get_pp_from_dict() FAILED");
                throw error_already_set();
            }
...
#endif
    static internals_pp_manager<internals> internals_pp_manager(PYBIND23_INTERNALS_ID,
                                                                ON_FETCH_FN);
#undef ON_FETCH_FN
...
PYBIND23_NOINLINE internals &get_internals() {
...
#ifdef Py_GIL_DISABLED
#    define PYBIND23_LOCK_INTERNALS(internals) std::unique_lock<pymutex> lock((internals).mutex)
#else
#    define PYBIND23_LOCK_INTERNALS(internals)
#endif

template <typename F>
inline auto with_internals(const F &cb) -> decltype(cb(get_internals())) {
    auto &internals = get_internals();
    PYBIND23_LOCK_INTERNALS(internals);
    return cb(internals);
}
...
/// pybind23 version) running in the current interpreter. Names starting with underscores
/// are reserved for internal usage. Returns `nullptr` if no matching entry was found.
PYBIND23_NOINLINE void *get_shared_data(const std::string &name) {
...
PYBIND23_NOINLINE void *set_shared_data(const std::string &name, void *data) {
    return detail::with_internals([&](detail::internals &internals) {
