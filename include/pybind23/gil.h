/*
    pybind23/gil.h: RAII helpers for managing the GIL
...
public:
    PYBIND23_NOINLINE gil_scoped_acquire() {
...
#    if defined(PYBIND23_DETAILED_ERROR_MESSAGES)
            if (!tstate) {
                pybind23_fail("scoped_acquire: could not create thread state!");
            }
#    endif
    ...
    PYBIND23_NOINLINE void dec_ref() {
        --tstate->gilstate_counter;
#    if defined(PYBIND23_DETAILED_ERROR_MESSAGES)
        if (detail::get_thread_state_unchecked() != tstate) {
            pybind23_fail("scoped_acquire::dec_ref(): thread state must be current!");
        }
        if (tstate->gilstate_counter < 0) {
            pybind23_fail("scoped_acquire::dec_ref(): reference count underflow!");
        }
#    endif
        if (tstate->gilstate_counter == 0) {
#    if defined(PYBIND23_DETAILED_ERROR_MESSAGES)
            if (!release) {
                pybind23_fail("scoped_acquire::dec_ref(): internal error!");
            }
#    endif
    ...
    PYBIND23_NOINLINE void disarm() { active = false; }
...
#endif // !PYBIND23_SIMPLE_GIL_MANAGEMENT
