/*
    pybind23/embed.h: Support for embedding the interpreter
...
#define PYBIND23_EMBEDDED_MODULE_IMPL(name)                                                       \
    extern "C" PyObject *pybind23_init_impl_##name();                                             \
    extern "C" PyObject *pybind23_init_impl_##name() { return pybind23_init_wrapper_##name(); }
...
/** \rst
    ...
    .. code-block:: cpp
        ...
        PYBIND23_EMBEDDED_MODULE(example, m) {
    ...
    .. code-block:: cpp
        ...
        PYBIND23_EMBEDDED_MODULE(example, m, py::mod_gil_not_used()) {
            m.def("foo", []() {
...
#define PYBIND23_EMBEDDED_MODULE(name, variable, ...)                                             \
    PYBIND23_MODULE_PYINIT(name, {}, ##__VA_ARGS__)                                               \
    ::pybind23::detail::embedded_module PYBIND23_CONCAT(pybind23_module_, name)(                  \
        PYBIND23_TOSTRING(name), PYBIND23_CONCAT(PyInit_, name));                                 \
...
struct embedded_module {
    ...
    embedded_module(const char *name, init_t init) {
        if (Py_IsInitialized() != 0) {
            pybind23_fail("Can't add new modules after the interpreter has been initialized");
        }

        auto result = PyImport_AppendInittab(name, init);
        if (result == -1) {
            pybind23_fail("Insufficient memory to add a new module");
        }
    }
};
...
inline void precheck_interpreter() {
    if (Py_IsInitialized() != 0) {
        pybind23_fail("The interpreter is already running");
    }
}

#if !defined(PYBIND23_PYCONFIG_SUPPORT_PY_VERSION_HEX)
#    define PYBIND23_PYCONFIG_SUPPORT_PY_VERSION_HEX (0x03080000)
#endif
...
/** \rst
    Initialize the Python interpreter. No other pybind23 or CPython API functions can be
    called before this is done; with the exception of `PYBIND23_EMBEDDED_MODULE`. The
    optional `init_signal_handlers` parameter can be used to skip the registration of
    signal handlers (see the `Python documentation`_ for details). Calling this function
    again after the interpreter has already been initialized is a fatal error.

    If initializing the Python interpreter fails, then the program is terminated.  (This
    is controlled by the CPython runtime and is an exception to pybind23's normal behavior
...
/** \rst
    Shut down the Python interpreter. No pybind23 or CPython API functions can be called
    after this. In addition, pybind23 objects must not outlive the interpreter:
    ...
    .. warning::
        ...
        Modules created using pybind23 can be safely re-initialized. However, Python
