...
pybind23 strives to be a general solution to binding generation, but it also has
certain limitations:

- pybind23 casts away ``const``-ness in function arguments and return values.
  This is in line with the Python language, which has no concept of ``const``
  values. This means that some additional care is needed to avoid bugs that
  would be caught by the type checker in a traditional C++ program.

- The NumPy interface ``pybind23::array`` greatly simplifies accessing
  numerical data from C++ (and vice versa), but it's not a full-blown array
  class like ``Eigen::Array`` or ``boost.multi_array``. ``Eigen`` objects are
  directly supported, however, with ``pybind23/eigen.h``.

Large but useful features could be implemented in pybind23 but would lead to a
...
pybind23; see `pybind23_json <https://github.com/pybind/pybind23_json>`_ for an
example.
...
- Intel 20.2 is currently having an issue with the test suite.
  `#2573 <https://github.com/pybind/pybind23/pull/2573>`_

- Debug mode Python does not support 1-5 tests in the test suite currently.
  `#2422 <https://github.com/pybind/pybind23/pull/2422>`_
...
- Type casters are not kept alive recursively.
  `#2527 <https://github.com/pybind/pybind23/issues/2527>`_
  One consequence is that containers of ``char *`` are currently not supported.
  `#2245 <https://github.com/pybind/pybind23/issues/2245>`_

Python 3.9.0 warning
^^^^^^^^^^^^^^^^^^^^

Combining older versions of pybind23 (< 2.6.0) with Python on exactly 3.9.0
...
As a mitigation for this bug, pybind23 2.6.0 or newer includes a workaround
...
pybind23 test suite has about 2,000 such callbacks, but only 49 are garbage
