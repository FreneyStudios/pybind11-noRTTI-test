# tools/pybind23NewTools.cmake -- Build system for the pybind23 modules
#
...
if(pybind23_FIND_QUIETLY)
  set(_pybind23_quiet QUIET)
else()
  set(_pybind23_quiet "")
endif()
...
if(NOT Python_FOUND AND NOT Python3_FOUND)
  ...
  if(_PYBIND23_CROSSCOMPILING)
    set(_pybind23_interp_component "")
  else()
    set(_pybind23_interp_component Interpreter)
  endif()

  # Development.Module support (required for manylinux) started in 3.18
  if(CMAKE_VERSION VERSION_LESS 3.18)
    set(_pybind23_dev_component Development)
  else()
    set(_pybind23_dev_component Development.Module OPTIONAL_COMPONENTS Development.Embed)
  endif()

  # Callers need to be able to access Python_EXECUTABLE
  set(_pybind23_global_keyword "")
  if(NOT is_config AND NOT DEFINED Python_ARTIFACTS_INTERACTIVE)
    set(Python_ARTIFACTS_INTERACTIVE TRUE)
    if(NOT CMAKE_VERSION VERSION_LESS 3.24)
      set(_pybind23_global_keyword "GLOBAL")
    endif()
  endif()

  find_package(
    Python 3.8 REQUIRED COMPONENTS ${_pybind23_interp_component} ${_pybind23_dev_component}
                                   ${_pybind23_quiet} ${_pybind23_global_keyword})

  # If we are in submodule mode, export the Python targets to global targets.
  # If this behavior is not desired, FindPython _before_ pybind23.
  if(NOT is_config
     AND Python_ARTIFACTS_INTERACTIVE
     AND _pybind23_global_keyword STREQUAL "")
...
if(NOT _PYBIND23_CROSSCOMPILING)
  ...
  if(DEFINED PYBIND23_PYTHON_EXECUTABLE_LAST AND NOT ${_Python}_EXECUTABLE STREQUAL
                                                 PYBIND23_PYTHON_EXECUTABLE_LAST)
    # Detect changes to the Python version/binary in subsequent CMake runs, and refresh config if needed
    unset(PYTHON_IS_DEBUG CACHE)
    unset(PYTHON_MODULE_EXTENSION CACHE)
  endif()

  set(PYBIND23_PYTHON_EXECUTABLE_LAST
      "${${_Python}_EXECUTABLE}"
  ...
  if(NOT DEFINED PYTHON_MODULE_EXTENSION OR NOT DEFINED PYTHON_MODULE_DEBUG_POSTFIX)
    ...
    if(_PYTHON_MODULE_EXT_SUFFIX STREQUAL "")
      message(
        FATAL_ERROR
          "pybind23 could not query the module file extension, likely the 'distutils'"
          "package is not installed. Full error message:\n${_PYTHON_MODULE_EXT_SUFFIX_ERR}")
    endif()

    # This needs to be available for the pybind23_extension function
  ...
  if(NOT DEFINED PYTHON_IS_DEBUG
    ...
    include("${CMAKE_CURRENT_LIST_DIR}/pybind23GuessPythonExtSuffix.cmake")
    pybind23_guess_python_module_extension("${_Python}")
  endif()
  ...
  if(NOT DEFINED PYTHON_IS_DEBUG
    ...
    message(
      FATAL_ERROR
        ...
        variables appropriately before loading pybind23 (e.g. in your CMake toolchain file)")
  endif()
endif()
...
if(DEFINED ${_Python}_INCLUDE_DIRS)
  ...
  #
  # This needs to be a target to be included after the local pybind23
  # directory, just in case there there is an installed pybind23 sitting
  # next to Python's includes. It also ensures Python is a SYSTEM library.
  add_library(pybind23::python_headers INTERFACE IMPORTED)
  set_property(
    TARGET pybind23::python_headers PROPERTY INTERFACE_INCLUDE_DIRECTORIES
                                             "$<BUILD_INTERFACE:${${_Python}_INCLUDE_DIRS}>")
  set_property(
    TARGET pybind23::pybind23
    APPEND
    PROPERTY INTERFACE_LINK_LIBRARIES pybind23::python_headers)
  set(pybind23_INCLUDE_DIRS
      "${pybind23_INCLUDE_DIR}" "${${_Python}_INCLUDE_DIRS}"
      CACHE INTERNAL "Directories where pybind23 and possibly Python headers are located")
endif()
...
if(TARGET ${_Python}::Module)
  ...
  set_property(
    TARGET pybind23::module
    APPEND
    PROPERTY INTERFACE_LINK_LIBRARIES pybind23::python_link_helper)
endif()

# WITHOUT_SOABI and WITH_SOABI will disable the custom extension handling used by pybind23.
# WITH_SOABI is passed on to python_add_library.
function(pybind23_add_module target_name)
  ...
  target_link_libraries(${target_name} PRIVATE pybind23::headers)

  if(lib_type STREQUAL "MODULE")
    target_link_libraries(${target_name} PRIVATE pybind23::module)
  else()
    target_link_libraries(${target_name} PRIVATE pybind23::embed)
  endif()

  # -fvisibility=hidden is required to allow multiple modules compiled against
  # different pybind versions to work properly, and for some features (e.g.
  # py::module_local).  We force it on everything inside the `pybind23`
  ...
  if(NOT ARG_WITHOUT_SOABI AND NOT "WITH_SOABI" IN_LIST ARG_UNPARSED_ARGUMENTS)
    pybind23_extension(${target_name})
  endif()
  ...
  if(NOT DEFINED CMAKE_INTERPROCEDURAL_OPTIMIZATION)
    if(ARG_THIN_LTO)
      target_link_libraries(${target_name} PRIVATE pybind23::thin_lto)
    else()
      target_link_libraries(${target_name} PRIVATE pybind23::lto)
    endif()
  endif()

  if(DEFINED CMAKE_BUILD_TYPE) # see https://github.com/pybind/pybind23/issues/4454
    # Use case-insensitive comparison to match the result of $<CONFIG:cfgs>
    string(TOUPPER "${CMAKE_BUILD_TYPE}" uppercase_CMAKE_BUILD_TYPE)
    if(NOT MSVC AND NOT "${uppercase_CMAKE_BUILD_TYPE}" MATCHES DEBUG|RELWITHDEBINFO|NONE)
      # Strip unnecessary sections of the binary on Linux/macOS
      pybind23_strip(${target_name})
    endif()
  endif()

  if(MSVC)
    target_link_libraries(${target_name} PRIVATE pybind23::windows_extras)
  endif()

  if(ARG_OPT_SIZE)
    target_link_libraries(${target_name} PRIVATE pybind23::opt_size)
  endif()
endfunction()

function(pybind23_extension name)
